...
**Tool:** `LTG_TOOL_ci_suite_v001.py` v1.0.0 (Kai Nakamura / Cycle 37)
...
**Status: PASS**
`LTG_TOOL_stub_linter_v001` — 159 file(s) — 159 PASS / 0 WARN / 0 ERROR
...
`LTG_TOOL_draw_order_lint_v002` v2.1.0 — 159 file(s) — 143 PASS / 16 WARN / 0 ERROR
...
**Status: WARN**
`LTG_TOOL_glitch_spec_lint_v001` v1.2.0 — 10 Glitch generator(s) — 4 PASS / 6 WARN (26 suppressed)

**Suppression list active:** `glitch_spec_suppressions.json` — 26 known false positives suppressed.

The 26 suppressions cover G005/G007 false positives on: `LTG_TOOL_color_verify_v001.py`, `LTG_TOOL_color_verify_v002.py`, `LTG_TOOL_fidelity_check_c24.py`, `LTG_TOOL_bg_glitch_storm_colorfix_v001.py`, `LTG_TOOL_bg_other_side_v002.py`, and G007 on legacy SF02/SF03 generators (v001–v005).

**Remaining WARN — 6 genuine issues across 3 files:**

- `LTG_TOOL_character_lineup_v004.py` (G006 ×7, G007 ×1) — non-Glitch skin tones trigger organic fill warning (Luma/Cosmo/Miri colors), G007 outline pattern. Known false positives from multi-character lineup context. **Recommend: add to suppression list.**
- `LTG_TOOL_character_lineup_v005.py` (G006 ×7, G007 ×1) — same as v004.
- `LTG_TOOL_character_lineup_v006.py` (G006 ×7, G007 ×1) — same as v004.
- `LTG_TOOL_character_lineup_v007.py` (G006 ×7, G007 ×1) — new C37 addition, same pattern.
- `LTG_TOOL_glitch_spec_lint_v001.py` (G002) — linter itself uses diamond geometry for spec test, rx=38 > ry=34. Known self-test false positive.
- `LTG_TOOL_glitch_color_model_v001.py` (G006 ×1) — SUNLIT_AMBER in swatch sheet triggers organic fill check.
- `LTG_TOOL_character_face_test_v001.py` (G003, G005, G006, G007) — legacy character face test with minimal Glitch implementation.
- `LTG_TOOL_glitch_turnaround_v001.py` (G007 ×1) — outline pattern not detected, legacy format.
- `LTG_TOOL_glitch_turnaround_v002.py` (G007 ×1) — outline pattern not detected, shadow color fixed in v002 but G007 pattern differs from spec expectation.
...
**Status: PASS**
`LTG_TOOL_spec_sync_ci_v001` — 5 character(s) — 0 P1 FAIL: none / 2 WARN: ['luma', 'cosmo']
...
**Status: PASS**
`LTG_TOOL_char_spec_lint_v001` — 3 character(s) — 3 PASS / 0 WARN / 0 FAIL

Luma, Cosmo, and Miri all pass all canonical spec checks (C001–C003, L001–L005, S001–S005, M001–M005).

Checks against: `LTG_TOOL_luma_expression_sheet_v009.py`, `LTG_TOOL_cosmo_expression_sheet_v004.py`, `LTG_TOOL_grandma_miri_expression_sheet_v004.py`.
...
*Tool: LTG_TOOL_ci_suite_v001.py v1.0.0*
