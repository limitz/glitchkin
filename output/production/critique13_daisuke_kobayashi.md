...
**Cycle:** 13
**Assets Reviewed:**
- `LTG_CHAR_luma_expressions_v007.png` / `LTG_TOOL_luma_expression_sheet_v007.py`
- `LTG_CHAR_luma_turnaround_v003.png` / `LTG_TOOL_luma_turnaround_v003.py`
- `LTG_CHAR_luma_lineup_v006.png` / `LTG_TOOL_character_lineup_v006.py`
- `LTG_COLOR_luma_color_model_v002.png` / `LTG_TOOL_luma_color_model_v002.py`
- `LTG_CHAR_glitch_expression_sheet_v003.png` / `LTG_TOOL_glitch_expression_sheet_v003.py`
...
- **P2 — SERIOUS: 6 body poses differentiate silhouettes, which is correct; but the per-expression hoodie color tint changes the character's palette per panel.** This is an undocumented design decision. If an animator uses the CURIOUS panel (cool blue hoodie) and the DETERMINED panel (warm amber hoodie) as on-model reference simultaneously, they will produce two visually different characters. Color tint variants are valid, but must be labeled and documented in the character spec. Currently not in `luma.md` or `character_sheet_standards_v001.md`.
