#!/usr/bin/env python3
"""
LTG_CHAR_glitch_turnaround_v001.py
Glitch — 4-View Character Turnaround v001
"Luma & the Glitchkin" — Cycle 23 / Maya Santos

4 views: FRONT, 3/4, SIDE, BACK
Canvas: 1600×700 (4 panels × 400w, single header bar)

Glitch design recap (see expression sheet docstring for full spec):
  - Diamond/rhombus body — angular, asymmetric
  - CORRUPT_AMBER #FF8C00 primary fill
  - HOT_MAGENTA scar lines
  - 3×3 pixel dual-eye system
  - Top + bottom spikes, two arm-spikes
  - No legs — corrupted confetti hover

Output: output/characters/main/turnarounds/LTG_CHAR_glitch_turnaround_v001.png

[Renamed from LTG_CHAR_glitch_turnaround_v001.py to LTG_TOOL_glitch_turnaround_v001.py
...
def draw_body_side(d, cx, cy, rx, ry):
    ...
    # Horizontal width at side ~40% of front rx
    rx_side = int(rx * 0.4)
    pts = [
        (cx,             cy - ry),
    ...
    # Edge highlight
    d.polygon([pts[0], (cx + rx_side//2, cy), pts[2]], fill=CORRUPT_AMB)
    d.polygon(pts, outline=LINE, width=3)
...
def draw_body_back(d, cx, cy, rx, ry):
    ...
    d.polygon(pts, outline=LINE, width=3)
    # Back-surface scar marks (structural lines showing corruption)
...
def build_turnaround():
    ...
    title = "GLITCH — Character Turnaround v001"
    try:
    ...
    sub = "Antagonist  |  GL-07 Corrupt Amber  |  Cycle 23  |  4-View  |  LTG_CHAR_glitch_turnaround_v001"
    try:
...
def main():
    ...
    out_path = os.path.join(out_dir, "LTG_CHAR_glitch_turnaround_v001.png")
    turnaround.save(out_path)
    print(f"Saved: {out_path}")
    print(f"  Size: {turnaround.size[0]}×{turnaround.size[1]}px")
