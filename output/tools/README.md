...
**Last updated:** 2026-03-29 (Cycle 34 — lineup_palette_audit_v001 added, Alex Chen; procedural_draw v1.5.0 scene_snapshot(), Rin Yamamoto; palette_warmth_lint_v002 + warmth_lint_config.json added, Sam Kowalski; char_spec_lint_v001 + draw_order_lint_v002 added, Kai Nakamura)
...
| `warmth_lint_config.json` | Sam Kowalski / Cycle 34 | **Config file for palette_warmth_lint_v002.** JSON: `{"warm_prefixes": ["CHAR-M"], "description": "..."}`. Edit `warm_prefixes` list to extend coverage to additional warm-guaranteed characters. Default ships with CHAR-M only. Path: `output/tools/warmth_lint_config.json`. | JSON (stdlib) |
...
| `LTG_TOOL_precritique_qa_v001.py` | Morgan Walsh / Cycle 34 | **Pre-critique QA pipeline entry point.** Chains all 6 existing QA tools into a single consolidated run: (1) `LTG_TOOL_render_qa_v001` on pitch PNGs, (2) `LTG_TOOL_color_verify_v002` on style frames, (3) `LTG_TOOL_proportion_verify_v001` on character turnarounds, (4) `LTG_TOOL_stub_linter_v001` on output/tools/, (5) `LTG_TOOL_palette_warmth_lint_v001` on master_palette.md, (6) `LTG_TOOL_glitch_spec_lint_v001` on generators. Output: `output/production/precritique_qa_c34.md` with PASS/WARN/FAIL counts per section, total score, flagged-items list. C34 baseline: WARN (PASS=150, WARN=36, FAIL=0). Exit: 0=PASS, 1=WARN, 2=FAIL. Usage: `python LTG_TOOL_precritique_qa_v001.py`. | Pillow, all QA tools |
...
- **Unlisted tools on disk:** 35 (33 LTG_TOOL_* + 2 non-standard names)
