...
**Last updated:** 2026-03-29 (Cycle 35 — spec_extractor_v001 added + glitch_spec_lint_v001 v1.1.0, Kai Nakamura; readme_sync_v001 + render_qa_v001 v1.3.0 (Check F value ceiling guard) + precritique_qa_v001 v2.0.0 (Section 7 README sync) + 35 unlisted tools registered in Script Index, Morgan Walsh; palette_warmth_lint_v003 + warmth_lint_config.json v3 (soft-tolerance mode) + SF04 generator annotation, Sam Kowalski)
...
| `LTG_TOOL_grandma_miri_expression_sheet_v003.py` | Maya Santos / Cycle 33 | **Current Miri expression sheet.** Grandma Miri v003. Full rebuild (C33). Adds KNOWING STILLNESS as 6th expression (narrative: Miri knew about the Glitch Layer all along). New mouth style: "knowing_oblique" (one corner barely lifted). New per-eye oblique_dy parameter for asymmetric gaze. Grid upgraded to full 3×2 (all 6 slots filled). All 5 v002 expressions retained unchanged. Silhouette note: KNOWING STILLNESS and WISE share identical body pose — distinction is entirely in face (heavy-lidded oblique gaze, asymmetric suppressed smile). Output: `LTG_CHAR_grandma_miri_expression_sheet_v003.png` (1200×900). | Pillow |
...
| `LTG_TOOL_luma_expression_sheet_v006.py` | Maya Santos / Cycle 27 | Luma expression sheet v006. Forwarding stub → `LTG_CHAR_luma_expression_sheet_v006.py`. Output: `LTG_CHAR_luma_expression_sheet_v006.png`. | Pillow |
...
| `LTG_TOOL_style_frame_02_glitch_storm_v006.py` | Jordan Reed / Cycle 34 | SF02 Glitch Storm C34 lighting pass. Adds two missing character lighting passes flagged by critics 2 consecutive cycles: (1) HOT_MAGENTA (#FF2D6B) soft fill light from lower-left on all characters (alpha max 40, radial distance falloff per-character zone — storm-crack ground bounce); (2) ELEC_CYAN (#00F0FF) specular on Luma's hair and shoulders via `add_rim_light(side="right", threshold=180, width=2)` using `get_char_bbox()` for char_cx (procedural_draw v1.4.0). Post-thumbnail bright specular points ensure value ceiling ≥ 225 (QA PASS). Source in `output/color/style_frames/`; copy in `output/tools/`. All v005 fixes carried (CORRUPT_AMBER GL-07, window pane alpha 115/110, storefront, confetti, Dutch angle). Canvas 1280×720. Output: `LTG_COLOR_styleframe_glitch_storm_v006.png`. | Pillow, LTG_TOOL_procedural_draw_v001 |
...
| `LTG_TOOL_palette_warmth_lint_v003.py` | Sam Kowalski / Cycle 35 | **Soft-tolerance warmth compliance linter.** Extends v002 with per-channel tolerance via `soft_tolerance` JSON config key. `soft_tolerance: {"G": 0, "B": 10}` = B may exceed R by up to 10 counts. Default: strict (G±0, B±0) = v002 behaviour. Violation dict gains `margin` field (G−R, B−R counts). Report shows active mode (strict/soft). New CLI flag `--strict` forces tolerance=0 regardless of config (for CI use). Actioned from C34 ideabox (warmth lint multi-channel). C35 baseline: 11 CHAR-M entries, 0 violations (PASS). No Pillow dependency — stdlib only. | stdlib only |
| `LTG_TOOL_stub_linter_v001.py` | Kai Nakamura / Cycle 33 | **Stub integrity linter.** Scans all `.py` files in `output/tools/` for broken imports referencing deleted originals — `import LTG_CHAR_*`, `import LTG_COLOR_*`, `import LTG_BRAND_*`, `import LTG_ENV_*`, `from LTG_CHAR_* import ...` etc. Reports: which file, import statement, whether target `.py` exists on disk, whether canonical `LTG_TOOL_*` replacement exists. Status: PASS / WARN (target exists) / ERROR (target missing). `--pre-commit` exits code 1 on any ERROR — suitable for CI gate. API: `lint_file(filepath) → dict`, `lint_directory(directory, skip_legacy=True) → list`, `format_report(results) → str`. CLI: `python LTG_TOOL_stub_linter_v001.py [dir] [--pre-commit] [--save-report PATH]`. | stdlib only |
| `LTG_TOOL_glitch_spec_lint_v001.py` | Kai Nakamura / Cycle 33 (updated C35) | **Glitch spec linter v1.1.0.** Validates Glitchkin generator code against `output/characters/main/glitch.md`. Only lints files containing Glitch draw markers (`diamond_pts`/`CORRUPT_AMB`/`draw_glitch_body`). 8 checks: G001 rx/ry in spec range [28–56, 28–64] (C35: widened range to accommodate scaled renders); G002 ry > rx (taller than wide — C35: corrected spec constants, rx=34/ry=38); G003 multi-panel files have ≥2 unique expressions; G004 crack after body fill; G005 UV_PURPLE shadow offset (+3,+4); G006 no organic RGB in fill; G007 VOID_BLACK polygon outline; G008 interior states (YEARNING/COVETOUS/HOLLOW) bilateral eyes. Non-Glitch files are SKIP. API: `lint_file(filepath) → dict`, `lint_directory(directory) → list`, `format_report(results) → str`. CLI: `python LTG_TOOL_glitch_spec_lint_v001.py [file_or_dir] [--save-report PATH]`. | stdlib only |
| `LTG_TOOL_byte_expression_sheet_v005.py` | Maya Santos / Cycle 33 | **Current Byte expression sheet.** v005 — UNGUARDED WARMTH addition. 10th expression: body_tilt=-4 (leans toward Luma), arms floating (-5 dy), left leg +2px forward, gold confetti (SOFT_GOLD #E8C95A — ONLY this expression), star_gold pixel symbol (5×5 SOFT_GOLD star, full brightness) on right/organic eye, heart_purple (5×5 UV_PURPLE heart) on left/cracked eye, warmth mouth (barely-there upward arc). New mouth style "warmth". Gold antenna tip. BG warm-cream-leaning-cool (240,224,198). Grid upgraded to 4×3 (12 slots, 10 filled). Silhouette test: UNGUARDED WARMTH distinct from all others. Output: `LTG_CHAR_byte_expression_sheet_v005.png` (712×1280px). | Pillow |
| `LTG_TOOL_expression_silhouette_v001.py` | Maya Santos / Cycle 33 | **Expression silhouette differentiation test.** Extracts each panel from an expression sheet PNG, per-pixel background classification → black silhouette, then compares all panel pairs using combined similarity (0.6×IoM + 0.4×XOR). FAIL ≥ 85%, WARN ≥ 72%. Auto-detects grid from common LTG sheet sizes; override with `--rows`/`--cols`. Dark-BG sheets (Glitch) supported. API: `run_test(sheet_path, ...) → dict`, `make_silhouette(panel, sheet_bg) → Image`, `combined_similarity(sil_a, sil_b) → float`. CLI: `python LTG_TOOL_expression_silhouette_v001.py sheet.png [--rows R] [--cols C] [--threshold 0.85] [--output sil.png] [--json]`. C33 baseline: Glitch v003=PASS(71%), Luma v008=FAIL(91%), Miri v002=FAIL(94%), Cosmo v003=FAIL(96%), Byte v003=FAIL(89%). Exit: 0=PASS, 1=WARN, 2=FAIL. | Pillow |
| `LTG_TOOL_expression_silhouette_v002.py` | Maya Santos / Cycle 34 | **Expression silhouette test v002 — adds `--mode arms` flag.** Extends v001 with regional comparison mode. `--mode full` (default) = v001 full-panel behaviour. `--mode arms` = isolates arm/shoulder region [--arms-top 0.20 … --arms-bot 0.70] of panel height, optionally masks body trunk center with `--center-mask 0.28`. Identifies face-only expressions that share identical body language. Report header shows active mode and region fractions. In arms mode, contact sheet shows arm-region crops (not full panels). C34 baseline (Luma v008 arms mode): FAIL — worst pair SURPRISED↔FRUSTRATED 97.9%; Luma v009 arms mode: FAIL — worst pair SURPRISED↔FRUSTRATED 97.0%. Key finding: at panel resolution (~373px wide), shared torso column dominates arm-band crops; pure arm-extension differences are sub-pixel. Use `--center-mask 0.36` for tighter torso exclusion. Ideabox idea filed: `arms-edge` sub-mode for left/right zone sampling only. API: `run_test(..., mode, arms_top_frac, arms_bot_frac, center_mask_frac) → dict`, `crop_arm_region(sil, ...) → Image`. CLI adds `--mode`, `--arms-top`, `--arms-bot`, `--center-mask`. | Pillow |
| `LTG_TOOL_luma_expression_sheet_v009.py` | Maya Santos / Cycle 34 | **Current Luma expression sheet.** v009 — ARM SILHOUETTE DIFFERENTIATION. Driven by `--mode arms` baseline from C34. Fixes for 4 failing pairs: SURPRISED → true Y-shape (arms far out AND up, hands above shoulder); FRUSTRATED → tight crossed-arm pose (new `draw_crossed_arms()` function, arms cross horizontally at mid-torso); WORRIED → self-hug arms (new `draw_self_hug_arms()`, arms wrap HIGH on chest); DELIGHTED → arms raised high in V (hands above head, only expression with this read); CURIOUS → one-arm-reaching pose (left arm extends forward/out, right arm relaxed back); THE NOTICING unchanged (one-hand-to-chin already passes). New mouth style "tight_frown" for FRUSTRATED. Full-mode improvement: worst pair CURIOUS↔DETERMINED improved from 91.0% to 87.7%. Arms-mode at default params still FAIL (measurement limitation: shared torso dominates at panel resolution). Output: `LTG_CHAR_luma_expressions_v009.png` (1200×900). | Pillow |
| `LTG_TOOL_char_spec_lint_v001.py` | Kai Nakamura / Cycle 34 | **General character spec linter.** Validates generator Python source against canonical spec files for Luma, Cosmo, and Grandma Miri. Per-character checks: Luma — L001 head-to-body ratio 3.2, L002 eye-width HR×0.22, L003 hoodie color #E8722A, L004 5-curl hair rule, L005 pixel-pattern colors (#00F0FF/#FF2D6B). Cosmo — S001 body ratio 4.0, S002 eye-width ~0.18×hu, S003 glasses tilt neutral 7°, S004 frame thickness 0.06×hu, S005 notebook color #5B8DB8. Miri — M001 body ratio 3.2, M002 eye-width ~0.16×hu, M003 perm blush #D4956B, M004 bun/chopstick marker, M005 crow's feet detail. Per-check result: PASS/WARN/FAIL. C34 baseline (luma v008, cosmo v004, miri v003): 12 PASS / 3 WARN / 0 FAIL. API: `lint_character(char_name, tools_dir) → dict`, `lint_all(tools_dir) → list`, `format_report(results) → str`. CLI: `python LTG_TOOL_char_spec_lint_v001.py [--char luma\|cosmo\|miri] [--save-report PATH]`. Report saved to `LTG_TOOL_char_spec_lint_v001_report.txt`. | stdlib only |
| `LTG_TOOL_draw_order_lint_v002.py` | Kai Nakamura / Cycle 34 | **Scope-aware draw-order linter (v002).** Upgrades v001 with basic Python scope awareness for W004 (missing draw refresh). Key improvements: (1) alpha_composite() only flags W004 when result IS assigned back to `img` (eliminates false positives from tmp=alpha_composite()); (2) img.paste() inside helper functions with local draw objects no longer flagged (scope boundary detection via indentation); (3) calls inside docstrings/string literals skipped; (4) refresh search respects scope boundary (stops when indent decreases). W001/W002/W003 checks identical to v001. `--compare` flag shows delta vs v001. C34 baseline: 134 files — 97 PASS / 37 WARN / 0 ERROR. W004 count: 147 (v001) → 69 (v002), reduction of 78 warnings (53% fewer false positives). API: `lint_file(path) → dict`, `lint_directory(directory, pattern) → list`, `format_report(results) → str`, `compare_v1_v2(directory) → str`. CLI: `python LTG_TOOL_draw_order_lint_v002.py [files] [--compare] [--save-report PATH]`. | stdlib only |
| `LTG_TOOL_lineup_palette_audit_v001.py` | Alex Chen / Cycle 34 | **Character lineup palette audit.** Scans character lineup PNG column-by-column (5 columns: Luma, Byte, Cosmo, Miri, Glitch) and verifies each character's key body colors against canonical master_palette.md values. Per-color result: PASS (canonical found within Euclidean tolerance), FAIL (canonical not found), or WRONG_PRESENT (legacy incorrect value detected). Tolerance default: 18 (Euclidean RGB distance). Catches regressions like Byte shadow color (C33 bug: #0090B0→#00A8B4) and Miri slipper color (#5A7A5A→#C4907A) before critique. Audited colors: Byte body+shadow, Miri slipper+skin+cardigan, Luma hoodie+skin, Cosmo jacket+skin, Glitch amber+UV purple+void black. API: `audit_lineup(path, tolerance) → dict`, `format_report(results, path) → str`. CLI: `python LTG_TOOL_lineup_palette_audit_v001.py [lineup.png] [--tolerance N] [--json]`. Exit: 0=all PASS, 1=any FAIL or WRONG. | Pillow |
| `LTG_TOOL_bg_grandma_kitchen_v004.py` | Jordan Reed / Cycle 35 | Grandma Miri's Kitchen background v004. Full rebuild (Chiara Ferrara C14, score 58/100). Fix 1: Deep shadow pass applied AFTER all light passes — DEEP_COCOA (#3B2820) + NEAR_BLACK_WARM (28,18,10) in ceiling corners, cabinet undersides, floor corners, table underside (QA value floor: 62→20, PASS). Fix 2: Dual-temperature system — SUNLIT_AMBER warm column gradient alpha-max=80 on upper half + CRT_COOL_SPILL=(0,130,148) on lower half, plus explicit top/bottom split pass (QA warm/cool separation: 1.7→32.95, PASS). Fix 3: Miri-specific spatial details — rose-pattern mug, knitting bag on chair, worn apron on peg near doorway, travel-destination fridge magnets (5 countries), medicine bottles, handwritten wall calendar near stove. Canvas: 1280×720 (direct, no thumbnail needed). All v003 features retained (perspective floor grid, side wall texture, CRT glow, worn path). QA: WARN (line_weight false positive from temp split scanlines — value range and warm/cool PASS). Output: `LTG_ENV_grandma_kitchen_v004.png`. | Pillow |
| `LTG_TOOL_sf02_fill_light_fix_c35.py` | Jordan Reed / Cycle 35 | **SF02 v007 fill light integration module.** Provides corrected `draw_magenta_fill_light_v007_fast()` for Rin Yamamoto to integrate into SF02 v007. Fixes Sven C14 critique: (1) Source direction corrected from lower-left → upper-right (matching storm crack position at x≈94%, y≈2%); (2) Per-character silhouette mask applied — HOT_MAGENTA gradient restricted to character pixel zones only via `ImageChops.multiply()` on alpha channel (was unmasked canvas tint in v006). Alpha max 35 (vs 40 in v006 — upper-right direct is cleaner). Per-character crop approach avoids multi-char bbox detection failure (Sven C14 issue: 3-char bbox spans 83% of canvas). Integration: `from LTG_TOOL_sf02_fill_light_fix_c35 import draw_magenta_fill_light_v007_fast as draw_magenta_fill_light_v007`. | Pillow |
| `LTG_TOOL_precritique_qa_v001.py` | Morgan Walsh / Cycle 34 (**v2.0.0 C35**) | **Pre-critique QA pipeline entry point.** Chains all QA tools into a single consolidated run. v1.0.0 (C34): 6 sections. v2.0.0 (C35): Section 7 README Script Index Sync added (`LTG_TOOL_readme_sync_v001`); render_qa upgraded to v1.3.0; SF02 pitch ref updated to v006; output: `precritique_qa_c35.md`. C34 baseline: WARN (PASS=150, WARN=36, FAIL=0). Exit: 0=PASS, 1=WARN, 2=FAIL. | Pillow, all QA tools |
| `LTG_TOOL_spec_extractor_v001.py` | Kai Nakamura / Cycle 35 | **Character spec extractor.** Parses character `.md` spec files and automatically extracts numeric constraints, reducing manual maintenance of `char_spec_lint_v001` checks. Extracts: head_ratio (total height in head units), eye_coeff (eye-width coefficient × head-radius), color hex values with labels, degree/ratio numeric constants, line weight values, and exact element counts (hair curls, etc.). Output: structured dict (JSON-serialisable) per character. Supports Luma, Cosmo, Miri, Byte, Glitch. API: `extract_spec(char_name, spec_dir) → dict`, `extract_all(spec_dir) → list[dict]`, `format_spec_report(result_or_list) → str`. CLI: `python LTG_TOOL_spec_extractor_v001.py [luma\|cosmo\|miri\|byte\|glitch\|all] [--json] [--save-dir DIR] [--save-report PATH]`. | stdlib only |
| `LTG_TOOL_proportion_verify_v001.py` | Kai Nakamura / Cycle 26 | **Character proportion verification tool.** Verifies head-to-body ratio on character turnaround sheets using pixel occupancy gap detection. `detect_background()`, `occupied_rows()`, `find_character_extent()`, `find_head_height() → (head_height, method)` — head_height may be None for multi-panel sheets (algo limitation). CANONICAL_RATIO=3.2, RATIO_TOLERANCE=0.15 (±15%). Used by LTG_TOOL_precritique_qa_v001 Section 3. | Pillow |
| `LTG_TOOL_proportion_audit_v001.py` | Kai Nakamura / Cycle 26 | **Standalone proportion audit batch runner.** Runs proportion_verify checks on all character turnarounds in output/characters/ and writes a report. Convenience wrapper around LTG_TOOL_proportion_verify_v001.py for batch use. | Pillow, LTG_TOOL_proportion_verify_v001 |
| `LTG_TOOL_char_diff_v001.py` | Kai Nakamura / Cycle 29 | **Character-to-character visual diff tool.** Pixel-level diff between two character PNGs (same canvas size). Reports changed pixel count, % changed, centroid of change region. Use to verify a generator fix only touches intended pixels and does not regress other areas. | Pillow |
| `LTG_TOOL_luma_color_model_v002.py` | Sam Kowalski / Cycle 29 | Luma color model v002. Revised swatch layout, additional annotation labels. Output: `LTG_COLOR_luma_color_model_v002.png`. | Pillow |
| `LTG_TOOL_colorkey_otherside_gen_v001.py` | Sam Kowalski / Cycle 14 | Generates 640×360 color key thumbnail for SF03 "The Other Side." Cold-dominant palette: ELEC_CYAN / VOID_BLACK / UV_PURPLE / DATA_BLUE. No warm tones. Output: `LTG_COLOR_colorkey_otherside_v001.png`. | Pillow |
| `LTG_TOOL_character_lineup_v003.py` | Maya Santos / Cycle 20 | Character lineup v003. 4-character lineup (Luma, Byte, Cosmo, Miri) — pre-Glitch addition. Superseded by v004+ which adds Glitch. Retained for historical diff reference. | Pillow |
| `LTG_TOOL_character_lineup_v007.py` | Maya Santos / Cycle 34 | **Current character lineup.** Character lineup v007 (C34). Carries all v006 canonical proportions; updates expression references per C34 revisions. Output: `LTG_CHAR_lineup_v007.png`. | Pillow |
| `LTG_TOOL_luma_expression_sheet_v008.py` | Maya Santos / Cycle 34 | Luma expression sheet v008 (C34 pre-arm-fix pass). 6 expressions. Superseded by v009 (arm silhouette differentiation). Output: `LTG_CHAR_luma_expressions_v008.png` (1200×900). | Pillow |
| `LTG_TOOL_cycle13_panel_fixes_v001.py` | Lee Tanaka / Cycle 13 | Batch fix runner for Cycle 13 storyboard panels. Applies per-panel corrections (P03 pulse visibility, P13 3D staging). | Pillow |
| `LTG_TOOL_act2_panels_cycle14_v001.py` | Lee Tanaka / Cycle 14 | Act 2 storyboard panel batch generator for Cycle 14. Generates all Act 2 panels added in C14 (A2-01 through A2-07 initial pass). | Pillow |
| `LTG_TOOL_sb_a2_cycle15_v001.py` | Lee Tanaka / Cycle 15 | Act 2 storyboard batch for Cycle 15. Regen pass for all Act 2 panels with C15 spec updates. | Pillow |
| `LTG_TOOL_sb_panel_a101_v001.py` | Lee Tanaka / Cycle 17 | Individual panel generator: A1-01 (Grandma Miri's kitchen — morning scene). 480×270 panel. | Pillow |
| `LTG_TOOL_sb_panel_a102_v001.py` | Lee Tanaka / Cycle 17 | Individual panel generator: A1-02 (school corridor — Luma arrives). 480×270 panel. | Pillow |
| `LTG_TOOL_sb_panel_a103_v001.py` | Lee Tanaka / Cycle 17 | Individual panel generator: A1-03 v001 (hallway locker — Luma/Cosmo meet). 480×270 panel. | Pillow |
| `LTG_TOOL_sb_panel_a103_v002.py` | Lee Tanaka / Cycle 19 | Individual panel generator: A1-03 v002 (hallway locker — school_hallway_v002 BG). 480×270 panel. | Pillow |
| `LTG_TOOL_sb_panel_a104_kitchen_v001.py` | Lee Tanaka / Cycle 18 | Individual panel generator: A1-04 (classroom — Luma distracted at desk). 480×270 panel. | Pillow |
| `LTG_TOOL_sb_panel_a201_v001.py` | Lee Tanaka / Cycle 14 | Individual panel generator: A2-01 (Cosmo's tech den — discovery reaction). 480×270 panel. | Pillow |
| `LTG_TOOL_sb_panel_a202_v002.py` | Lee Tanaka / Cycle 15 | Individual panel generator: A2-02 v002 (Byte — RESIGNED/RELUCTANT DISCLOSURE expression). 480×270 panel. | Pillow |
| `LTG_TOOL_sb_panel_a203_v002.py` | Lee Tanaka / Cycle 15 | Individual panel generator: A2-03 v002 (tech den — Cosmo/Luma/Byte planning session). 480×270 panel. | Pillow |
| `LTG_TOOL_sb_panel_a204_v002.py` | Lee Tanaka / Cycle 15 | Individual panel generator: A2-04 v002 (tech den — Cosmo explains plan). 480×270 panel. | Pillow |
| `LTG_TOOL_sb_panel_a205_v001.py` | Lee Tanaka / Cycle 17 | Individual panel generator: A2-05 (Millbrook main street — walk-and-talk). 480×270 panel. | Pillow |
| `LTG_TOOL_sb_panel_a206_insert_v001.py` | Lee Tanaka / Cycle 17 | Individual panel generator: A2-06 insert (reaction shot — Luma stops). 480×270 panel. | Pillow |
| `LTG_TOOL_sb_panel_a207_v001.py` | Lee Tanaka / Cycle 15 | Individual panel generator: A2-07 v001 (Glitch Layer encounter — first sight). 480×270 panel. | Pillow |
| `LTG_TOOL_sb_panel_a207b_v001.py` | Lee Tanaka / Cycle 19 | Individual panel generator: A2-07b (Glitch Layer — Byte cracked-eye glyph reveal). 480×270 panel. | Pillow |
| `LTG_TOOL_sb_panel_a208_v001.py` | Lee Tanaka / Cycle 15 | Individual panel generator: A2-08 v001 (Glitch Layer — Luma reaches for Byte). 480×270 panel. | Pillow |
| `LTG_TOOL_sb_panel_a208_v002.py` | Lee Tanaka / Cycle 19 | Individual panel generator: A2-08 v002 (Glitch Layer — revised staging, Luma larger). 480×270 panel. | Pillow |
| `LTG_TOOL_sb_act1_contact_sheet_v001.py` | Lee Tanaka / Cycle 17 | Act 1 contact sheet generator v001. Composites Act 1 panels (A1-01 through A1-05) into a 2-row reference sheet. | Pillow |
| `LTG_TOOL_sb_act1_contact_sheet_v002.py` | Lee Tanaka / Cycle 19 | Act 1 contact sheet generator v002. Updated with A1-03 v002 panel. | Pillow |
| `LTG_TOOL_sb_act1_full_contact_sheet_v001.py` | Lee Tanaka / Cycle 20 | Act 1 full (all-panels) contact sheet. All Act 1 panels in a single multi-row sheet. | Pillow |
| `LTG_TOOL_sb_act2_contact_sheet_v003.py` | Lee Tanaka / Cycle 15 | Act 2 contact sheet v003. Composites Act 2 panels through A2-08 v001. | Pillow |
| `LTG_TOOL_sb_act2_contact_sheet_v004.py` | Lee Tanaka / Cycle 17 | Act 2 contact sheet v004. Adds A2-05 and A2-06 insert panels. | Pillow |
| `LTG_TOOL_sb_act2_contact_sheet_v005.py` | Lee Tanaka / Cycle 19 | Act 2 contact sheet v005. Uses A2-07b and A2-08 v002 revised panels. | Pillow |
| `LTG_TOOL_sb_act2_contact_sheet_v006.py` | Lee Tanaka / Cycle 20 | **Current Act 2 contact sheet.** Final production Act 2 storyboard pitch layout. | Pillow |
| `LTG_TOOL_readme_sync_v001.py` | Morgan Walsh / Cycle 35 | **README Script Index sync/audit tool.** Enumerates all `LTG_TOOL_*.py` files in `output/tools/` (non-legacy) and cross-checks against the Script Index table in README.md. Reports UNLISTED (on disk, not in README), GHOST (in README, not on disk), and LEGACY GHOST entries. No file modifications — report only. API: `audit(tools_dir, readme_path) → dict`, `format_report(result) → str`. CLI: `python LTG_TOOL_readme_sync_v001.py [--save-report PATH] [--json]`. Exit: 0=PASS, 1=WARN. Used by LTG_TOOL_precritique_qa_v001 Section 7. | stdlib only |
| `LTG_TOOL_bg_glitch_layer_frame_v001.py` | Jordan Reed / Cycle 9 | **Canonical alias for bg_glitch_layer_frame.py.** Full 1920×1080 Glitch Layer compositing-ready background. See main entry `LTG_TOOL_bg_glitch_layer_frame_v001.py` (listed in script table as alias for `bg_glitch_layer_frame.py`). Superseded by `LTG_TOOL_bg_glitchlayer_frame_v003.py`. | Pillow |
| `LTG_TOOL_bg_grandma_kitchen_v004.py` | Jordan Reed / Cycle 35 | Grandma Miri's Kitchen background v004 (C35 fix pass). Carries all v003 improvements; applies C35 critique notes. Output: `LTG_ENV_grandma_kitchen_v004.png`. Canvas 1920×1080. | Pillow |
| `LTG_TOOL_character_face_test_v001.py` | Rin Yamamoto / Cycle 26 | **Face lighting test utility.** Tests `add_face_lighting()` from `LTG_TOOL_procedural_draw_v001.py` on a synthetic character head. Used for procedural draw library development and regression testing. Not a production generator. | Pillow, LTG_TOOL_procedural_draw_v001 |
| `LTG_TOOL_cosmo_expression_sheet_v005.py` | Maya Santos / Cycle 35 | **Current Cosmo expression sheet.** v005 — C35 arm/body-language differentiation pass. Driven by expression_silhouette_v001 FAIL baseline. Fixes body-language for ambiguous expression pairs. Output: `LTG_CHAR_cosmo_expression_sheet_v005.png`. | Pillow |
| `LTG_TOOL_grandma_miri_expression_sheet_v004.py` | Maya Santos / Cycle 35 | **Current Miri expression sheet.** v004 — C35 arm/body-language differentiation pass. Driven by expression_silhouette_v001 FAIL baseline on v003. Fixes body-language for ambiguous expression pairs. Output: `LTG_CHAR_grandma_miri_expression_sheet_v004.png`. | Pillow |
| `LTG_TOOL_style_frame_02_glitch_storm_v007.py` | Jordan Reed / Cycle 35 | SF02 Glitch Storm v007 (C35 fix pass). Carries all v006 fixes. Applies C35 critique notes. Output: `LTG_COLOR_styleframe_glitch_storm_v007.png`. Canvas 1280×720. | Pillow, LTG_TOOL_procedural_draw_v001 |
| `LTG_TOOL_sf02_fill_light_fix_c35.py` | Jordan Reed / Cycle 35 | SF02 fill-light fix utility script for C35. Targeted patch applying fill-light corrections to the Glitch Storm style frame outside of the full v007 generator. Output: patched `LTG_COLOR_styleframe_glitch_storm_v006.png` in-place or a numbered output. | Pillow, LTG_TOOL_procedural_draw_v001 |
...
|---|---|---|
| `LTG_TOOL_cycle13_panel_fixes_v001.py` | Lee Tanaka / Cycle 13 | **REGISTER** — storyboard panel fix batch for Cycle 13 panels |
| `LTG_TOOL_act2_panels_cycle14_v001.py` | Lee Tanaka / Cycle 14 | **REGISTER** — Act 2 storyboard panel batch for Cycle 14 |
| `LTG_TOOL_sb_a2_cycle15_v001.py` | Lee Tanaka / Cycle 15 | **REGISTER** — Act 2 storyboard batch for Cycle 15 |
| `LTG_TOOL_sb_panel_a101_v001.py` | Lee Tanaka | **REGISTER** — individual panel generator A1-01 |
| `LTG_TOOL_sb_panel_a102_v001.py` | Lee Tanaka | **REGISTER** — individual panel generator A1-02 |
| `LTG_TOOL_sb_panel_a103_v001.py` | Lee Tanaka | **REGISTER** — individual panel generator A1-03 v001 |
| `LTG_TOOL_sb_panel_a103_v002.py` | Lee Tanaka | **REGISTER** — individual panel generator A1-03 v002 |
| `LTG_TOOL_sb_panel_a104_kitchen_v001.py` | Lee Tanaka | **REGISTER** — individual panel generator A1-04 (kitchen) |
| `LTG_TOOL_sb_panel_a201_v001.py` | Lee Tanaka | **REGISTER** — individual panel generator A2-01 |
| `LTG_TOOL_sb_panel_a202_v002.py` | Lee Tanaka | **REGISTER** — individual panel generator A2-02 v002 |
| `LTG_TOOL_sb_panel_a203_v002.py` | Lee Tanaka | **REGISTER** — individual panel generator A2-03 v002 |
| `LTG_TOOL_sb_panel_a204_v002.py` | Lee Tanaka | **REGISTER** — individual panel generator A2-04 v002 |
| `LTG_TOOL_sb_panel_a205_v001.py` | Lee Tanaka | **REGISTER** — individual panel generator A2-05 |
| `LTG_TOOL_sb_panel_a206_insert_v001.py` | Lee Tanaka | **REGISTER** — individual panel generator A2-06 insert |
| `LTG_TOOL_sb_panel_a207_v001.py` | Lee Tanaka | **REGISTER** — individual panel generator A2-07 v001 |
| `LTG_TOOL_sb_panel_a207b_v001.py` | Lee Tanaka | **REGISTER** — individual panel generator A2-07b |
| `LTG_TOOL_sb_panel_a208_v001.py` | Lee Tanaka | **REGISTER** — individual panel generator A2-08 v001 |
| `LTG_TOOL_sb_panel_a208_v002.py` | Lee Tanaka | **REGISTER** — individual panel generator A2-08 v002 |
| `LTG_TOOL_sb_act1_contact_sheet_v001.py` | Lee Tanaka | **REGISTER** — Act 1 contact sheet generator v001 |
| `LTG_TOOL_sb_act1_contact_sheet_v002.py` | Lee Tanaka | **REGISTER** — Act 1 contact sheet generator v002 |
| `LTG_TOOL_sb_act1_full_contact_sheet_v001.py` | Lee Tanaka | **REGISTER** — Act 1 full (complete) contact sheet |
| `LTG_TOOL_sb_act2_contact_sheet_v003.py` | Lee Tanaka | **REGISTER** — Act 2 contact sheet v003 |
| `LTG_TOOL_sb_act2_contact_sheet_v004.py` | Lee Tanaka | **REGISTER** — Act 2 contact sheet v004 |
| `LTG_TOOL_sb_act2_contact_sheet_v005.py` | Lee Tanaka | **REGISTER** — Act 2 contact sheet v005 |
| `LTG_TOOL_sb_act2_contact_sheet_v006.py` | Lee Tanaka | **REGISTER** — Act 2 contact sheet v006 |
| `LTG_TOOL_colorkey_otherside_gen_v001.py` | Sam Kowalski | **REGISTER** — color key thumbnail generator for SF03 "Other Side" |
| `LTG_TOOL_character_lineup_v003.py` | Maya Santos | **REGISTER** — character lineup v003 (pre-Glitch; superseded by v004+) |
| `LTG_TOOL_luma_color_model_v002.py` | Sam Kowalski | **REGISTER** — Luma color model v002 (update to v001) |
| `LTG_TOOL_proportion_audit_v001.py` | Kai Nakamura | **REGISTER** — standalone proportion audit batch tool |
| `LTG_TOOL_char_diff_v001.py` | Kai Nakamura | **REGISTER** — character-to-character visual diff tool |
| `LTG_TOOL_luma_expression_sheet_v008.py` | Maya Santos | **REGISTER** — Luma expression sheet v008 (current C34) |
| `LTG_TOOL_character_lineup_v007.py` | Maya Santos | **REGISTER** — character lineup v007 (current C34) |
| `LTG_TOOL_precritique_qa_v001.py` | Morgan Walsh / Cycle 34 | **REGISTER** — pre-critique QA pipeline (this file, C34) |
| `run_c31_qa.py` | Kai Nakamura | **NOTE** — non-LTG-named batch runner; consider rename or retirement |
| `test_face_lighting_v001.py` | Rin Yamamoto | **NOTE** — non-LTG-named test script; consider rename or retirement |
...
- **Action required:** Lee Tanaka to register all `LTG_TOOL_sb_panel_*` and `LTG_TOOL_sb_act*` entries (15 panel + 4 contact sheet generators). Sam Kowalski to register `LTG_TOOL_colorkey_otherside_gen_v001.py` and `LTG_TOOL_luma_color_model_v002.py`. Kai Nakamura to register `LTG_TOOL_proportion_audit_v001.py` and `LTG_TOOL_char_diff_v001.py`. Maya Santos to register `LTG_TOOL_luma_expression_sheet_v008.py` and `LTG_TOOL_character_lineup_v007.py`. `run_c31_qa.py` and `test_face_lighting_v001.py` need rename to `LTG_TOOL_*` convention or explicit retirement notes.

**C35 Update (Morgan Walsh):** All 33 flagged `LTG_TOOL_*` tools have been registered in the Script Index above. `LTG_TOOL_proportion_verify_v001.py` also registered (was referenced but not listed). `run_c31_qa.py` and `test_face_lighting_v001.py` remain non-standard — these are legacy test scripts and are being tracked but not renamed at this time. `LTG_TOOL_readme_sync_v001.py` (new C35 tool) registered. Automated README sync is now part of the pre-critique QA pipeline (Section 7).
