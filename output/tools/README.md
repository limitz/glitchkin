...
**Last updated:** 2026-03-30 (Cycle 33 — procedural_draw v1.4.0 get_char_bbox(), Rin Yamamoto; palette_warmth_lint_v001 added, Sam Kowalski; stub_linter_v001 + glitch_spec_lint_v001 added, Kai Nakamura)
...
| `LTG_TOOL_procedural_draw_v001.py` | Rin Yamamoto / Cycle 26 (v1.4.0 C33) | **Procedural drawing library.** `wobble_line()`, `wobble_polygon()`, `variable_stroke()`, `add_rim_light(side, char_cx=None)`, `silhouette_test()`, `value_study()`, `add_face_lighting()`, `get_char_bbox()`. v1.4.0 (C33): `get_char_bbox(img, threshold=128) → (cx, cy, left, top, right, bottom)` — auto-detects character silhouette bounding box from bright pixels. Callers pass `char_cx=get_char_bbox(img)[0]` to `add_rim_light()` without manually tracking head_cx. Eliminates canvas-midpoint bug risk for any character at any x-position. Backward compatible (existing calls unchanged). v1.3.0 (C32): `add_rim_light()` gains optional `char_cx` parameter. | Pillow |
