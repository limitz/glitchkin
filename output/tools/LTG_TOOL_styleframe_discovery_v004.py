...
def draw_luma_body(draw, luma_cx, luma_base_y, facing_monitor_x):
    ...
    for i in range(7):
        ppx = luma_x - torso_half_w + lean_offset + rng_px.randint(sp(4), torso_half_w * 2 - sp(8))
        ppy = torso_top + rng_px.randint(sp(4), sp(30))
    ...
    # Warm lit arm
    arm_shoulder_x = luma_x - sp(20) + lean_offset
    arm_shoulder_y = torso_top + sp(30)
    arm_target_x = facing_monitor_x
    arm_target_y = torso_top + sp(60)
    elbow_x = (arm_shoulder_x + arm_target_x) // 2 + sp(20)
    elbow_y = arm_shoulder_y - sp(40)
    ...
    draw.ellipse([hand_cx - sp(14), hand_cy - sp(14), hand_cx + sp(14), hand_cy + sp(14)], fill=CYAN_SKIN)
    draw.line([(hand_cx - sp(8), hand_cy - sp(10)), (hand_cx - sp(14), hand_cy - sp(22))],
              fill=CYAN_SKIN, width=sp(7))
    draw.line([(hand_cx, hand_cy - sp(12)), (hand_cx + sp(2), hand_cy - sp(24))],
              fill=CYAN_SKIN, width=sp(7))
    draw.line([(hand_cx + sp(8), hand_cy - sp(10)), (hand_cx + sp(16), hand_cy - sp(20))],
              fill=CYAN_SKIN, width=sp(7))
...
def draw_luma_head(img, draw, cx, cy, scale=1.0):
    """Draw Luma's head with procedural quality (wobble outline + variable stroke)."""
    ...
    # Wobble outline on head silhouette (procedural quality — C29)
    num_pts = 20
    head_pts = []
    ...
    draw.line(l_brow, fill=HAIR_COLOR, width=p(5))
    ...
    draw.arc([cx - p(45) + m_off, cy + p(18), cx + p(45) + m_off, cy + p(70)],
             start=5, end=175, fill=LINE, width=p(4))
    ...
    draw.arc([cx - p(42) + m_off, cy + p(22), cx + p(42) + m_off, cy + p(65)],
             start=7, end=173, fill=LINE, width=p(2))

    # Blush — warm peach (C29: matches SF04 v003 corrected color)
    ...
    draw.arc([cx - p(60), cy - p(195), cx - p(10), cy - p(140)], start=30, end=200, fill=HAIR_COLOR, width=p(8))
    draw.arc([cx - p(20), cy - p(190), cx + p(40), cy - p(130)], start=10, end=190, fill=HAIR_COLOR, width=p(7))
...
def generate():
    ...
    draw = ImageDraw.Draw(img)  # Refresh after ghost paste operations in draw_background
    ...
    # STEP 5: Luma's Head (with wobble outline + variable stroke — C29 procedural quality)
    head_cx = body_data["head_cx"]
    head_cy = body_data["head_cy"]
    draw, head_r = draw_luma_head(img, draw, head_cx, head_cy, scale=0.92)

    # STEP 5b: Face lighting — warm lamp from upper-left (domestic real-world scene)
    # Light comes from the lamp at upper-left (warm domestic lighting)
    add_face_lighting(
        img,
        face_center=(head_cx, head_cy),
        face_radius=(head_r, head_r),
        light_dir=(-1.0, -1.0),               # upper-left lamp source
        shadow_color=SKIN_SH,                  # warm-dark shadow on right/lower face
        highlight_color=SKIN_HL,               # warm highlight on lit upper-left
        seed=500,
    )
    draw = ImageDraw.Draw(img)  # Refresh after face lighting
