...
Diego Vargas, Storyboard Artist — Cycle 37
...
Beat sequence (pitch emotional arc):
  ...
  P4 — Luma leans in — glitch colors bleed into warm room — MCU PUSH-IN
  P5 — Byte presses against the glass from inside — ECU TWO-WORLD TOUCH
  P6 — Luma's expression: THE NOTICING — MCU reaction / pitch emotional core

Output: LTG_SB_pilot_cold_open_v001.png
...
OUTPUT_PATH = os.path.join(OUTPUT_DIR, "LTG_SB_pilot_cold_open_v001.png")
...
def draw_p2_crt_spotting(img, rng):
    ...
    # Luma — OTS foreground silhouette (left edge, backlit)
    sil_x = int(W * 0.10)
    sil_y = int(H * 0.25)
    # Shoulder / back silhouette
    draw.polygon([
        (0, H),
        (0, int(H * 0.42)),
        (int(W * 0.08), sil_y),
...
def draw_p3_pixel_glitch(img, rng):
    ...
    # THE GLITCH — pixel shape forming in center
    cx, cy = W // 2, int(H * 0.48)
    # Burst of electric pixels radiating from center — Glitchkin emerging
    pixel_cluster = [
        (0, 0, 6, 6, ELEC_CYAN),
        (-8, -4, -2, 2, ELEC_CYAN),
        (4, -6, 10, 0, HOT_MAGENTA),
        (-6, 4, 0, 10, ELEC_CYAN),
        (8, 6, 14, 12, HOT_MAGENTA),
        (-12, 0, -6, 6, ELEC_CYAN),
        (2, -14, 8, -8, HOT_MAGENTA),
        (-4, -10, 2, -4, ELEC_CYAN),
        (10, -2, 16, 4, HOT_MAGENTA),
        (-16, -4, -10, 2, UV_PURPLE),
        (16, 8, 22, 14, ELEC_CYAN),
        (-2, 14, 4, 20, UV_PURPLE),
    ]
    for (px0, py0, px1, py1, pc) in pixel_cluster:
        draw.rectangle([cx + px0, cy + py0, cx + px1, cy + py1], fill=pc)
    ...
    # Loose pixel trails radiating outward
    for angle_deg in range(0, 360, 30):
        ...
        for dist in [28, 44, 60]:
            ...
            if 0 <= trail_x < W and 0 <= trail_y < H:
                ...
                sz = max(1, 4 - dist // 22)
                draw.rectangle([trail_x, trail_y, trail_x + sz, trail_y + sz], fill=col)

    # Screen edge vignette (bezel border)
    for bw in range(1, 8):
        alpha = int(180 * (1 - bw / 8))
    ...
    draw.text((4, 14), "pixel shape FORMS — something moves inside", font=load_font(8), fill=(0, 200, 210))
...
def draw_p4_luma_leans_in(img, rng):
    ...
    # Cyan bleed from right (CRT off-screen right)
    ...
    # CRT TV (right side, mostly off-frame — just visible)
    tv_rx = int(W * 0.80)
    tv_ty = int(H * 0.28)
    tv_bw = W - tv_rx + 20
    tv_bh = int(H * 0.42)
    ...
    # Pixel burst escaping screen edge
    for py in range(tv_ty + 12, tv_ty + tv_bh - 12, 10):
        col = ELEC_CYAN if py % 20 < 10 else HOT_MAGENTA
        draw.rectangle([tv_rx - 4, py, tv_rx, py + 6], fill=col)
    draw = ImageDraw.Draw(img)

    # Luma — MCU from mid-chest up, LEANING FORWARD toward TV
    luma_cx = int(W * 0.40)
    luma_top = int(H * 0.05)
    # Body forward lean — hoodie (angled toward right)
    body_pts = [
    ...
    for px in range(luma_cx - 18, luma_cx + 18, 6):
        for py in range(int(H * 0.45), int(H * 0.60), 6):
            if rng.random() > 0.55:
                col = rng.choice([(0, 180, 200), (180, 160, 80), LUMA_HOODIE])
    ...
    draw.arc([head_cx + 3, head_cy - 22, head_cx + 20, head_cy - 10],
             start=200, end=340, fill=LINE_DARK, width=2)
    # Cyan rim light on Luma from TV (right side of face/body)
    ...
    draw.text((4, 14), "Cyan bleeds warm room — two worlds touching", font=load_font(8),
              fill=(120, 200, 195))
...
def draw_p6_the_noticing(img, rng):
    ...
    # Cyan fill light (from CRT off left)
    ...
    for px in range(luma_cx - 28, luma_cx + 28, 7):
        for py in range(int(H * 0.59), int(H * 0.74), 7):
            if rng.random() > 0.5:
                col = rng.choice([ELEC_CYAN, (180, 160, 60), LUMA_HOODIE])
    ...
    # Brows: asymmetric — left raised (wonder), right slightly furrowed (fear)
    # Mouth: slight open — caught mid-breath

    # Left eye (wide, raised brow side — more wonder)
    le_cx = luma_cx - 22
    ...
    draw.ellipse([le_cx - 6, le_cy - 6, le_cx + 6, le_cy + 6],
                 fill=(28, 20, 12))
    # Cyan light catch in iris
    draw.ellipse([le_cx - 5, le_cy - 8, le_cx + 3, le_cy],
                 fill=(0, 185, 210), outline=None)
    # Specular
    draw.ellipse([le_cx + 2, le_cy - 9, le_cx + 7, le_cy - 4],
                 fill=(255, 255, 255))
    # Left brow — high arch (wonder)
    draw.arc([le_cx - 18, le_cy - 28, le_cx + 18, le_cy - 10],
             start=195, end=345, fill=LUMA_HAIR, width=3)

    # Right eye (slightly narrower, furrowed brow side — slight apprehension)
    re_cx = luma_cx + 22
    ...
    draw.ellipse([re_cx - 5, re_cy - 5, re_cx + 5, re_cy + 5],
                 fill=(28, 20, 12))
    draw.ellipse([re_cx - 4, re_cy - 7, re_cx + 2, re_cy - 1],
                 fill=(0, 185, 210), outline=None)
    draw.ellipse([re_cx + 2, re_cy - 8, re_cx + 6, re_cy - 4],
                 fill=(255, 255, 255))
    # Right brow — straighter with slight inner furrow
    ...
    # Cyan screen light on Luma's face (left-side fill)
    add_glow(img, luma_cx - head_rx - 10, luma_head_cy, 50, ELEC_CYAN, steps=4, max_alpha=28)
    draw = ImageDraw.Draw(img)
    ...
    draw.text((4, H - 28), "EMOTIONAL CORE — wonder + apprehension",
...
PANELS = [
    ...
    },
    {
        ...
        "cam":   "MCU — push-in / cyan bleeds warm room",
    ...
    },
    {
        ...
        "cam":   "MCU — low angle / warm rim + cyan fill",
        "beat":  "Luma's expression: wonder, fear, can't look away. Pitch core.",
        "arc":   ARC_CORE,
    },
]
...
def make_contact_sheet():
    ...
    draw.text((MARGIN, 26), "Key Beat Storyboard — 6 Panels — Diego Vargas, C37",
    ...
    draw.text((MARGIN, footer_y),
              "LTG_SB_pilot_cold_open_v001  |  output/storyboards/  |  ≤1280px",
...
if __name__ == "__main__":
    make_contact_sheet()
    print("LTG_SB_pilot_cold_open_v001 generation complete (Cycle 37).")
