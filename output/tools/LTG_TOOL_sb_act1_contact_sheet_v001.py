#!/usr/bin/env python3
"""
LTG_TOOL_sb_act1_contact_sheet_v001.py
Act 1 Cold Open Contact Sheet v001 — Cycle 18
Lee Tanaka, Storyboard Artist

Composites the 4 Act 1 kitchen cold open panels into a single contact sheet.
Arc-colored borders and beat labels.
Header: "Act 1 — Cold Open"

Panel sequence (arc order):
  A1-01 — ESTABLISHING  (QUIET arc — kitchen, morning, CRT glow visible)
  A1-02 — ARRIVAL       (CURIOUS arc — Luma enters, spots TV)
  A1-03 — DISCOVERY     (CURIOUS arc — Luma at screen, pixel shape)
  A1-04 — FIRST CONTACT (SURPRISED arc — Byte appears, reaction)
...
Layout: 2×2 grid (2 columns, 2 rows) with header and footer.

Output:
  /home/wipkat/team/output/storyboards/LTG_SB_act1_coldopen_contact_sheet_v001.png
"""
...
OUTPUT_PATH = os.path.join(OUTPUT_DIR, "LTG_SB_act1_coldopen_contact_sheet_v001.png")

os.makedirs(OUTPUT_DIR, exist_ok=True)

# Panel sequence: (filepath, beat_label, arc_label, arc_color_rgb)
PANEL_SEQUENCE = [
    (PANELS_DIR + "/LTG_SB_act1_panel_a101_v001.png",
     "A1-01\nestablishing", "ESTABLISHING", (200, 170, 80)),
    (PANELS_DIR + "/LTG_SB_act1_panel_a102_v001.png",
     "A1-02\narrival",      "ARRIVAL",      (0,  200, 210)),
    (PANELS_DIR + "/LTG_SB_act1_panel_a103_v001.png",
     "A1-03\ndiscovery",    "DISCOVERY",    (0,  210, 220)),
    (PANELS_DIR + "/LTG_SB_act1_panel_a104_v001.png",
...
THUMB_W  = 300
THUMB_H  = 168   # 16:9-ish approximation of 800×600 panel at thumb scale
MARGIN   = 20
GUTTER   = 12
LABEL_H  = 32
HEADER_H = 52
...
def ensure_panels_exist():
    ...
    generators = [
        "/home/wipkat/team/output/tools/LTG_TOOL_sb_panel_a101_v001.py",
        "/home/wipkat/team/output/tools/LTG_TOOL_sb_panel_a102_v001.py",
        "/home/wipkat/team/output/tools/LTG_TOOL_sb_panel_a103_v001.py",
        "/home/wipkat/team/output/tools/LTG_TOOL_sb_panel_a104_kitchen_v001.py",
    ]
    outputs = [
        PANELS_DIR + "/LTG_SB_act1_panel_a101_v001.png",
        PANELS_DIR + "/LTG_SB_act1_panel_a102_v001.png",
...
def make_contact_sheet():
    ...
    draw.text((MARGIN, 34), '"Luma & the Glitchkin"  |  Kitchen Cold Open  |  Cycle 18  |  Lee Tanaka',
              font=font_sub, fill=TEXT_ARC)

    # Arc flow annotation (right side of header)
    ...
    for row in range(ROWS):
        for col in range(COLS):
            ...
            # Short label (right side)
            label_lines = short_label.split('\n')
            for li, ll in enumerate(label_lines):
                draw.text((thumb_x + THUMB_W - 85, label_y + 2 + li * 11),
                          ll, font=font_label, fill=TEXT_LABEL)

            panel_idx += 1

    # ── Arc flow connector line ───────────────────────────────────────────────
    # Horizontal arrow between panels in same row to show flow
    # Row 1 connector (A1-01 → A1-02)
    ...
    # Vertical connector between rows (A1-02 → A1-03 wrap)
    ...
    draw.text((MARGIN, foot_y + 8),
              'Act 1 Cold Open — 4 panels — QUIET → CURIOUS → SURPRISED arc',
              font=font_foot, fill=TEXT_FOOTER)
    draw.text((TOTAL_W - 280, foot_y + 8),
              "LTG_SB_act1_coldopen_contact_sheet_v001  |  Cycle 18",
...
if __name__ == "__main__":
    ensure_panels_exist()
    make_contact_sheet()
    print("Act 1 cold open contact sheet generation complete (Cycle 18).")
