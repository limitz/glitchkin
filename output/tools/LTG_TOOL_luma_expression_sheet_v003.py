#!/usr/bin/env python3
"""
LTG_TOOL_luma_expression_sheet_v003.py
Luma Expression Sheet — v003 Fix Pass
...
Squint test after fix:
  CURIOUS:    default hair + asymmetric brow offset (marginal pass — preserved)
...
def build_sheet():
    ...
    title = "LUMA — Expression Sheet v003  |  Luma & the Glitchkin"
    sub   = ("Designer: Maya Santos  |  Cycle 19 Fix Pass  |  "
             "DELIGHTED arms anchor added  |  Brow weight fixed 5px→2px  |  6/6 squint pass")
...
if __name__ == "__main__":
    ...
    out_path = os.path.join(out_dir, "LTG_CHAR_luma_expression_sheet_v003.png")
    sheet    = build_sheet()
    sheet.save(out_path)
    print(f"Saved: {os.path.abspath(out_path)}")
    print(f"Canvas: {sheet.size[0]}x{sheet.size[1]}")
    print("Fixes applied:")
    print("  - DELIGHTED: arms raised above shoulders (unique silhouette vs SURPRISED)")
    print("  - Brow weight: 10px→4px at 2x render (5px→2px output, interior structure)")
    print("  - All 6 expressions re-verified against squint test")
