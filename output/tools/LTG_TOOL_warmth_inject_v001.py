...
the extension:  LTG_ENV_tech_den_v004.png → LTG_ENV_tech_den_v004_warminjected.png

CLI USAGE
---------
  python3 LTG_TOOL_warmth_inject_v001.py <input.png> [--mode warm|cool|auto] [--alpha INT]
...
-----------
  ...
  on the _warminjected output to confirm the pass:
    ...
    python3 output/tools/LTG_TOOL_warmth_inject_v001.py \\
        output/backgrounds/environments/LTG_ENV_tech_den_v004.png \\
        --mode auto

    python3 output/tools/LTG_TOOL_render_qa_v001.py \\
        output/backgrounds/environments/LTG_ENV_tech_den_v004_warminjected.png
...
def main():
    parser = argparse.ArgumentParser(
        description="LTG_TOOL_warmth_inject_v001 — warm/cool injection for environment PNGs"
    )
