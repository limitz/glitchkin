...
Procedural quality additions (v002):
  - wobble_polygon() on hair silhouette, hoodie torso silhouette
  - wobble_line() on window frame edges, floor baseboard, desk edge
  - variable_stroke() on Luma head perimeter and Byte body perimeter
  - add_rim_light() on Luma (cool cyan from monitor side, right)
  - add_face_lighting() on Luma (warm light from upper-left window)
...
OUTPUT_PATH = "/home/wipkat/team/output/color/style_frames/LTG_COLOR_styleframe_luma_byte_v002.png"
...
BYTE_TEAL       = (  0, 212, 232)   # GL-01b — Byte body fill / cool rim
DEEP_CYAN       = (  0, 168, 180)
...
def generate():
    ...
    # Step 4: add_rim_light on Luma — cool cyan from monitor (right side)
    # Use a moderate threshold to target the warm skin/hoodie areas
    add_rim_light(img, threshold=170, light_color=BYTE_TEAL, width=3)
    draw = ImageDraw.Draw(img)
    ...
    # Save — already at 1280x720, no thumbnail needed
