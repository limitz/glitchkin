...
C39 addition:
  ...
  - Warm amber/gold catch-light only — no GL palette values
  - Reads as decorative ornament at thumbnail; echoes Glitch Layer diamond
    motif on rewatch (secondary visual plant, connects Grandma Miri / elder
    Glitchkin "Miri" and the diamond body geometry of the Glitch Layer)
...
# ── Layer 3b: Diamond-Crystal Figurine (C39 — secondary visual plant) ─────────

def draw_crystal(img, vp_x, vp_y, bw_left, bw_top, bw_right, bw_bot):
    """
    Small diamond-faceted glass crystal on the bookshelf top shelf,
    right side of the shelf. Warm amber/gold catch-light only —
    no GL palette values (no ELEC_CYAN, no UV_PURPLE).

    At full size: clearly a diamond-faceted crystal ornament.
    At thumbnail scale: reads as a small gold-lit decorative object.

    Secondary visual plant: echoes the diamond body geometry of the Glitch Layer.
    (The bookcase spans bc_x1=bw_left to bc_x2=W*0.36.
    Shelves: shelf_h = (bc_y2 - bc_y1) // 4.
    Top shelf sits at bc_y1 + shelf_h*0/5 → shelf_bot = bc_y1 + shelf_h.)
    """
    draw = ImageDraw.Draw(img)

    # Bookcase geometry (mirrored from draw_bookcase)
    bc_x1 = bw_left
    bc_x2 = int(W * 0.36)
    bc_y1 = bw_top + 10
    bc_y2 = bw_bot
    ...
    # Place crystal on the TOP shelf, near the right edge of the bookcase
    # Top shelf bot y = bc_y1 + shelf_h
    top_shelf_bot = bc_y1 + shelf_h - 6    # sitting on the shelf plank

    # Crystal center — right side of bookcase, close to CRT side
    crys_cx = bc_x2 - 22    # ~439px with W=1280
    crys_cy = top_shelf_bot  # base sits on shelf

    # Crystal dimensions — small enough to miss at thumbnail, clear at full size
    crys_half_w = 9     # half-width of widest point
    crys_top_h  = 14    # height above widest point (upper diamond)
    crys_bot_h  = 8     # height below widest point (lower culet)

    # Diamond facet polygon points (classic diamond silhouette):
    #   crown top point (girdle), table flat top, culet bottom point
    #   We draw a classic gem: top point, two upper girdle edges, two lower girdle, bottom
    apex_top = (crys_cx, crys_cy - crys_top_h)
    girdle_l  = (crys_cx - crys_half_w, crys_cy)
    girdle_r  = (crys_cx + crys_half_w, crys_cy)
    culet     = (crys_cx, crys_cy + crys_bot_h)

    # Table facet (flat top, slightly inset from apex)
    table_w = crys_half_w - 3
    table_y = crys_cy - crys_top_h + 5
    table_l  = (crys_cx - table_w, table_y)
    table_r  = (crys_cx + table_w, table_y)

    # Glass body fill — very light warm grey-gold (looks like glass)
    CRYSTAL_BASE    = (218, 204, 178)   # pale warm grey-gold body
    CRYSTAL_SHADOW  = (168, 148, 118)   # shadow facet side
    CRYSTAL_DARK    = (128, 106,  76)   # deep culet shadow
    CRYSTAL_CATCH   = (252, 210, 110)   # warm amber/gold catch-light (ONLY warm)
    CRYSTAL_SPECULAR = (255, 248, 225)  # near-white specular hot spot
    CRYSTAL_OUTLINE = ( 90,  68,  40)   # warm dark outline (not pure black)

    # ── Upper half (crown) — split into left and right facets ──────────
    # Right crown facet (lit side — catch-light direction: upper-right)
    draw.polygon([apex_top, table_r, girdle_r], fill=CRYSTAL_BASE)
    # Left crown facet (shadow side)
    draw.polygon([apex_top, table_l, girdle_l], fill=CRYSTAL_SHADOW)
    # Table facet (top flat surface)
    draw.polygon([apex_top, table_l, table_r], fill=CRYSTAL_BASE)

    # ── Lower half (pavilion) — three facets ───────────────────────────
    # Right pavilion facet (lit)
    draw.polygon([girdle_r, culet, girdle_l], fill=CRYSTAL_SHADOW)
    # Draw slightly lighter right pavilion to suggest lit/shadow split
    mid_pav = ((girdle_r[0] + culet[0]) // 2, (girdle_r[1] + culet[1]) // 2)
    draw.polygon([girdle_r, mid_pav, culet], fill=CRYSTAL_DARK)

    # ── Catch-light — warm amber/gold, upper-right of gem ─────────────
    # Small warm triangle catch-light on the right crown facet
    catch_pts = [
        (crys_cx + 2, crys_cy - crys_top_h + 4),
        (crys_cx + crys_half_w - 1, crys_cy - 3),
        (crys_cx + 3, crys_cy - 4),
    ]
    img = alpha_overlay_poly(img, catch_pts, CRYSTAL_CATCH, 200)
    draw = ImageDraw.Draw(img)

    # Specular hot spot — tiny bright dot near top-right
    spec_x = crys_cx + 4
    spec_y = crys_cy - crys_top_h + 6
    draw.ellipse([spec_x - 2, spec_y - 2, spec_x + 2, spec_y + 2],
                 fill=CRYSTAL_SPECULAR)

    # ── Outline — ties it together, ensures it reads at full size ──────
    draw.line([apex_top, girdle_r, culet, girdle_l, apex_top],
              fill=CRYSTAL_OUTLINE, width=1)
    draw.line([girdle_l, girdle_r], fill=CRYSTAL_OUTLINE, width=1)
    draw.line([table_l, table_r], fill=CRYSTAL_OUTLINE, width=1)
    draw.line([apex_top, table_l], fill=CRYSTAL_OUTLINE, width=1)
    draw.line([apex_top, table_r], fill=CRYSTAL_OUTLINE, width=1)
    draw.line([table_l, girdle_l], fill=CRYSTAL_OUTLINE, width=1)
    draw.line([table_r, girdle_r], fill=CRYSTAL_OUTLINE, width=1)

    # ── Cast shadow on shelf ───────────────────────────────────────────
    shadow_pts = [
        (crys_cx - crys_half_w + 2, crys_cy + 1),
        (crys_cx + crys_half_w,     crys_cy + 1),
        (crys_cx + crys_half_w + 3, crys_cy + 5),
        (crys_cx - crys_half_w - 1, crys_cy + 4),
    ]
    img = alpha_overlay_poly(img, shadow_pts, CRYSTAL_DARK, 120)
    draw = ImageDraw.Draw(img)

    # Return crystal center for reporting
    return crys_cx, crys_cy, img
...
def main():
    ...
    # Layer 4b: Diamond-crystal figurine on bookshelf (C39 — secondary visual plant)
    crys_cx, crys_cy, img = draw_crystal(img, vp_x, vp_y, bw_left, bw_top, bw_right, bw_bot)
    print(f"[Hana] Crystal center: ({crys_cx}, {crys_cy})")
