...
Cycle: 46 (created), 48 (re-calibrated), 49 (landmark detector integration)
"""
...
# ── Optional landmark detector import ──────────────────────────────────────
_LANDMARK_DETECTOR_AVAILABLE = False
try:
    from LTG_TOOL_face_landmark_detector import (
        detect_landmarks as _detect_landmarks,
        get_backend_name as _get_landmark_backend,
        get_available_backends as _get_landmark_backends,
        FaceLandmarks as _FaceLandmarks,
    )
    _LANDMARK_DETECTOR_AVAILABLE = True
except ImportError:
    pass
...
def _landmark_to_face_detection(lm):
    """Convert a FaceLandmarks object to a FaceDetection for backward compat.

    The FaceLandmarks object already computes the same ratios, so we
    copy them into a FaceDetection-shaped wrapper.
    """
    # Build eyes list from landmark points (for has_full_detection)
    eyes = []
    if lm.left_eye:
        # Fake eye bbox: center +/- 5px
        eyes.append((int(lm.left_eye.x - 5), int(lm.left_eye.y - 5), 10, 10))
    if lm.right_eye:
        eyes.append((int(lm.right_eye.x - 5), int(lm.right_eye.y - 5), 10, 10))

    smile_bbox = None
    if lm.mouth_left and lm.mouth_right:
        mx = int(lm.mouth_left.x)
        my = int(lm.mouth_left.y - 5)
        mw = int(abs(lm.mouth_right.x - lm.mouth_left.x))
        mh = 10
        smile_bbox = (mx, my, mw, mh)

    nose_bbox = None
    if lm.nose_tip:
        nose_bbox = (int(lm.nose_tip.x - 5), int(lm.nose_tip.y - 5), 10, 10)

    det = FaceDetection(lm.filepath, lm.face_bbox, eyes, smile_bbox, nose_bbox)
    # Override computed ratios with the landmark detector's more precise values
    det.ratios = dict(lm.ratios)
    return det


def detect_faces_with_landmarks(filepath, backend="auto", debug=False):
    """Detect faces using the landmark detector if available, else Haar.

    Returns list of FaceDetection objects (backward compatible).
    """
    if _LANDMARK_DETECTOR_AVAILABLE:
        landmarks = _detect_landmarks(filepath, backend=backend)
        return [_landmark_to_face_detection(lm) for lm in landmarks]
    else:
        return detect_faces_in_image(filepath, debug=debug)
...
def generate_report(all_detections, stats, comparisons, face_dir, body_dir, output_path):
    ...
    lines.append("")
    if _LANDMARK_DETECTOR_AVAILABLE:
        lines.append(f"**Detection method:** LTG_TOOL_face_landmark_detector ({_get_landmark_backend('auto')} backend)")
    else:
    ...
    lines.append("")
    lines.append("---")
    lines.append(f"*Report generated by LTG_TOOL_face_metric_calibrate.py — C49*")
    lines.append("")
...
def run_calibration(face_dir, body_dir, output_path, debug=False, backend="auto"):
    """Run the full calibration pipeline.

    Args:
        face_dir: Directory with face reference images.
        body_dir: Directory with body reference images.
        output_path: Path for output report.
        debug: Save annotated debug PNGs.
        backend: Landmark detection backend: "auto", "dlib", "haar".
            "auto" uses the landmark detector if available, else Haar cascades.

    Returns (all_detections, stats, comparisons, report_path).
    """
    all_detections = []

    # Select detection function based on backend preference
    use_landmarks = _LANDMARK_DETECTOR_AVAILABLE and backend != "haar"

    def _detect(filepath):
        if use_landmarks:
            return detect_faces_with_landmarks(filepath, backend=backend, debug=debug)
        else:
            return detect_faces_in_image(filepath, debug=debug)
...
def main():
    ...
    parser.add_argument("--backend", default="auto", choices=["auto", "dlib", "haar"],
                        help="Landmark detection backend (default: auto — uses best available)")

    args = parser.parse_args()

    print(f"Face Metric Calibration Tool — LTG C49")
    print(f"Face dir:   {args.face_dir}")
    print(f"Body dir:   {args.body_dir}")
    print(f"Output:     {args.output}")
    print(f"Debug PNGs: {'Yes' if args.debug else 'No'}")
    print(f"Landmark detector: {'Available' if _LANDMARK_DETECTOR_AVAILABLE else 'Not available (using Haar cascades)'}")
    if _LANDMARK_DETECTOR_AVAILABLE:
        print(f"Backend: {_get_landmark_backend(args.backend)}")
        print(f"Available backends: {', '.join(_get_landmark_backends())}")
    print()
