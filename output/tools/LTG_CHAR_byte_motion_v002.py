"""
LTG_CHAR_byte_motion_v002.py
Ryo Hasegawa / Cycle 38
Motion Spec Sheet — BYTE (v002)

FIXES from C37/C38 critique:
1. P2 CRACK SCAR SIDE: Crack scar moved to viewer's RIGHT (cx + offset) to match
   Byte's cracked eye which is also at viewer's right (cx + eye_gap). Spec:
   "Right eye (viewer's left)" — this refers to Byte's anatomical right = viewer's left
   for a full frontal view. However in the draw_byte_body code, the "cracked right eye"
   was being drawn at cx + eye_gap (viewer's right), so the scar MUST match that side.
   Fix: crack scar now at cx + int(actual_bw * 0.55) + tx (matching cracked eye x side).
2. P2 GLOW RADIUS ANNOTATION: Added recommended max glow radius annotation to APPROACH
   panel. Max recommended: 1.5× body width (body half-axis). Animators must not exceed
   this or Byte's visual presence doubles and competes with scene elements.

Output: output/characters/motion/LTG_CHAR_byte_motion_v002.png
...
GLOW_CYAN        = (  0, 232, 248)
MAX_GLOW_COL     = (255, 200,  60)   # amber warning for max glow radius annotation
...
def draw_byte_body(img_or_draw, draw_obj, cx, cy, bw, bh,
    ...
    v002 FIX: crack scar moved to cx + side (viewer right), matching cracked eye position.
    """
    ...
    # HOT_MAGENTA crack scar — v002 FIX: moved to viewer's RIGHT (cx + offset)
    # to match cracked eye which is drawn at cx + eye_gap (viewer right)
    ...
    if phase == 0:  # normal — left: soft_gold cross/dot, right: clean dot
        # Left eye (warm/organic) — viewer's left = cx - eye_gap
        for ey_off in range(3):
            for ex_off in range(3):
                ...
                draw.rectangle([cx - eye_gap + tx + ex_off * pixel_sz,
                                ...
                                fill=col)
        # Right eye (cracked) — viewer's right = cx + eye_gap (MATCHES SCAR SIDE in v002)
        for ey_off in range(3):
            for ex_off in range(3):
...
def draw_panel2_approach(img, draw):
    ...
    v002 FIX: Added max glow radius annotation.
    Recommended max: 1.5× body half-width (bw). At full approach (−22°),
    peak glow = 1.5×bw radius. Do NOT exceed this value.
    """
    col = 2
    px, py = panel_origin(col)
    ...
    s1_y = base_y
    s1_bw = 22
    draw.ellipse([s1_x - s1_bw, s1_y - 20, s1_x + s1_bw, s1_y + 20],
                 fill=BYTE_TEAL, outline=LINE_COLOR, width=2)
    ...
    # --- v002 FIX: MAX GLOW RADIUS ANNOTATION ---
    # Max recommended glow radius = 1.5× body half-width
    # At s3_x with bw3=30: max_r = 1.5 * 30 = 45
    max_r = int(bw3 * 1.5)
    # Draw dashed max-radius circle
    steps = 32
    for i in range(0, steps, 2):  # draw every other segment = dashed effect
        a0 = 2 * math.pi * i / steps
        a1 = 2 * math.pi * (i + 1) / steps
        x0 = int(s3_x + max_r * math.cos(a0)) + tx3
        y0 = int(s3_y + max_r * math.sin(a0))
        x1 = int(s3_x + max_r * math.cos(a1)) + tx3
        y1 = int(s3_y + max_r * math.sin(a1))
        draw.line([(x0, y0), (x1, y1)], fill=MAX_GLOW_COL, width=1)
    # Annotation label
    draw.text((s3_x + max_r + tx3 + 3, s3_y - 8), "MAX", fill=MAX_GLOW_COL)
    draw.text((s3_x + max_r + tx3 + 3, s3_y + 2), f"r={max_r}px", fill=MAX_GLOW_COL)
    draw.text((s3_x + max_r + tx3 + 3, s3_y + 12), "(1.5×bw)", fill=MAX_GLOW_COL)
    ...
    draw.text((px + 8, py + PANEL_H - 55), "Glow halo: ELEC_CYAN, 3-ring falloff", fill=ELEC_CYAN)
    draw.text((px + 8, py + PANEL_H - 44), "MAX RADIUS: 1.5×bw (dashed circle)", fill=MAX_GLOW_COL)
...
def main():
    ...
    draw.text((PAD, 8), "BYTE — Motion Spec Sheet v002", fill=LABEL_TEXT)
    draw.text((PAD, 22), "RYO HASEGAWA  |  Luma & the Glitchkin  |  C38  |  FIXES: crack scar side, glow max radius", fill=(120, 165, 180))
    # legend strip
    legend_x = W - 280
    ...
    draw.text((legend_x + 140, 20), "- - MAX glow radius", fill=MAX_GLOW_COL)
    ...
    out_path = os.path.join(out_dir, "LTG_CHAR_byte_motion_v002.png")
    img.save(out_path)
