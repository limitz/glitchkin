...
Luma — 4-View Character Turnaround v002
"Luma & the Glitchkin" — Cycle 25 / Maya Santos

v002 UPDATE: Cycle 10 turnaround did not match Act 2 proportions.
This version matches current Luma character proportions:
  - A-line trapezoid hoodie (wider at hem) — established Cycle 5
  - Oversized sneakers with chunky sole + cyan laces — established Cycle 5
...
Palette: canonical luma_color_model.md values.

Output: output/characters/main/turnarounds/LTG_CHAR_luma_turnaround_v002.png
"""
...
def render_front(draw, cx, base_y):
    ...
    draw.polygon(torso_pts, outline=LINE, width=5)
    # Hood rim
...
def render_three_quarter(draw, cx, base_y):
    ...
    draw.arc([cx - int(h * 0.24), hair_top + int(h * 0.05),
             ...
             start=200, end=340, fill=HAIR_HL, width=3)
    draw.ellipse([cx - head_r, head_cy - head_r,
                  cx + head_r, head_cy + head_r], outline=LINE, width=6)
...
def render_side(draw, cx, base_y):
    ...
    draw.line([torso_front, torso_top, torso_front, torso_bot], fill=LINE, width=5)
    draw.line([torso_back, torso_top, torso_back, torso_bot], fill=LINE, width=5)
    draw.line([torso_back, torso_top, torso_front, torso_top], fill=LINE, width=4)
    draw.line([torso_back, torso_bot, torso_front, torso_bot], fill=LINE, width=4)
...
def render_back(draw, cx, base_y):
    ...
    draw.polygon(hood_pts, outline=LINE, width=4)
...
def build_turnaround():
    ...
    title = "LUMA — Character Turnaround v002  |  Luma & the Glitchkin  |  Cycle 25 / Maya Santos"
    sub   = "v002: Updated to Act 2 proportions — A-line hoodie, oversized sneakers, 3.2 heads"
...
def main():
    ...
    out_path = os.path.join(out_dir, "LTG_CHAR_luma_turnaround_v002.png")
    img.save(out_path)
    print(f"Saved: {out_path}")
    print(f"  Size: {img.size[0]}x{img.size[1]}px")
    print("  v002: Act 2 proportions — A-line hoodie, oversized sneakers (cyan laces), 3.2 heads")
