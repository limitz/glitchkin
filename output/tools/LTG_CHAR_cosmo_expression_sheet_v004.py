...
v004 Cycle 22 (Maya Santos): Critique 10 fix — Dmitri Volkov Priority 3.
  SKEPTICAL arm posture redesigned:
    ...
    The goal: SKEPTICAL reads as "contracted inward" at thumbnail, NOT "arms raised".
...
EXPRESSIONS = [
    ...
    # ── SKEPTICAL v004 FIX — arms lowered to near-neutral + body_squash ─────────
    # PROBLEM (v003): arm_l_dy=-14, arm_r_dy=-10 pushed arms UPWARD -> reads as
    #   mild SURPRISED at thumbnail. Dmitri C10: "redesign as lateral/inward containment."
    # FIX (Option B per task brief): arm_l_dy=2, arm_r_dy=2 (near-neutral, not raised).
    #   body_squash=0.92 (compressed/contained torso read).
    #   Asymmetric brow (l_raise=18, r_raise=0) remains as face signal.
    #   Result: SKEPTICAL reads as "contracted inward" at thumbnail, not "arms raised".
    {
        "name":     "SKEPTICAL",
        "body_data": {
            "arm_l_dy": 2, "arm_r_dy": 2,        # v004: near-neutral (was -14,-10 = arms up)
            "body_tilt": 6, "body_squash": 0.92, # v004: squash = compressed/contained read
            "notebook_show": True, "notebook_open": False,
        },
        "panel_bg": BG_SKEPTIC,
        "glasses_tilt": 9,
...
def _draw_cosmo_body(draw, cx, body_top_y, hu, body_data):
    ...
    # ── LEAN FORMULA FIX (Cycle 19) ─────────────────────────────────────────
    # v001/v002 bug: tilt_off = int(body_tilt * 0.4) → only ~2.4px for tilt=6 (invisible)
    # Fix: multiplier increased to 2.5 → 6 * 2.5 = 15px (clearly visible at thumbnail)
...
def generate_cosmo_expression_sheet(output_path):
    ...
    draw.text((PAD, 14),
              "COSMO — Expression Sheet — Luma & the Glitchkin  |  v004  |  SKEPTICAL arm posture fix",
    ...
    print("  SKEPTICAL: arm_l_dy/arm_r_dy -14/-10 -> 2/2 (near-neutral, not raised)")
    print("  SKEPTICAL: body_squash 1.0 -> 0.92 (compressed/contained read)")
    print("  Lean fix from v003 preserved: tilt_off multiplier 2.5 still active")
