#!/usr/bin/env python3
"""
LTG_TOOL_bg_grandma_kitchen_v002.py — Grandma Miri's Kitchen Background v002
"Luma & the Glitchkin" — Background & Environment Design
Artist: Jordan Reed | Cycle 21
...
v002 improvements (Cycle 21, responding to Takeshi Murakami C9 feedback B+):
  1. Floor linoleum pattern: subtle tile grid lines (2px, alpha ~25) at
     regular 60×60px intervals. Warm-path worn section brighter (lighter
     rectangle from doorway to stove zone, alpha 20). Floor now reads as
     old lived-in linoleum, not flat plane.
  2. Upper wall texture: very subtle horizontal stripes at low opacity
     (~12–15%) alternating warm cream/buff with base wall color. Gives
     wall visual identity — textured but not patterned. Feels like period
     wallpaper rather than flat paint.
  3. CRT glow intensity: radius increased + second wider glow ring at lower
     alpha for atmospheric ambient presence — gentle invitation to look
     through doorway. Note: ltg_render_lib.py not yet available (Kai
     Nakamura building this cycle) so scanline_overlay() not applied.
...
Output: /home/wipkat/team/output/backgrounds/environments/LTG_ENV_grandma_kitchen_v002.png
"""
...
def draw_floor_tiles(draw, bw_left, bw_bot, bw_right, vp_x, vp_y):
    """Old linoleum floor tiles — alternating warm/dark pattern, worn path."""
    floor_top_y = bw_bot

    # Tile grid lines (perspective)
    num_rows = 12
    for i in range(1, num_rows + 1):
        ...
        if fy >= H:
            break
        left_x = max(0, int(bw_left - (bw_left * t)))
        draw.line([(0, fy), (W, fy)], fill=GROUT_LINE, width=1)

    num_cols = 16
    for i in range(num_cols + 1):
        tx = i / num_cols
        bx = int(tx * W)
        draw.line([(vp_x, floor_top_y), (bx, H)], fill=GROUT_LINE, width=1)

    # Checkerboard color variation (every other column)
    col_rng = random.Random(61)
    for ci in range(num_cols):
        if ci % 2 == 1:
            x0 = int((ci / num_cols) * W)
            x1 = int(((ci + 1) / num_cols) * W)
            # Dark tile column
            for ri in range(1, num_rows):
                t0 = ((ri - 1) / num_rows) ** 1.6
                t1 = (ri / num_rows) ** 1.6
                y0 = floor_top_y + int(t0 * (H - floor_top_y))
                y1 = floor_top_y + int(t1 * (H - floor_top_y))
                if ri % 2 == 0:
                    draw.rectangle([x0, y0, x1, y1], fill=FLOOR_TILE_DARK)

    # Worn path: from sink (center-back) toward table/camera
    # Lighter than base tile, centered around x=40-55% of frame
    worn_cx = int(W * 0.48)
    for y in range(floor_top_y, H):
        t = (y - floor_top_y) / (H - floor_top_y)
        worn_w = int(80 + t * 180)
        x0 = worn_cx - worn_w // 2
        x1 = worn_cx + worn_w // 2
        draw.line([(x0, y), (x1, y)], fill=FLOOR_WORN_PATH)


def draw_floor_linoleum_overlay(img, bw_left, bw_bot, bw_right, vp_x, vp_y):
    """
    v002 ADDITION: Subtle linoleum tile grid overlay.
    Draws a regular 60×60px tile grid at very low alpha (~25) over the
    floor area to make the linoleum pattern clearly readable.
    Also adds a brighter worn-path rectangle from doorway area to stove
    zone (lighter value rectangle, alpha 20) to suggest 40 years of
    traffic pattern.
    """
    floor_top_y = bw_bot
    TILE_SIZE = 60

    # ── Regular tile grid overlay (flat-plane, 2px lines, alpha 25) ─────────
    # Horizontal grid lines across floor
    grid_overlay = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    gd = ImageDraw.Draw(grid_overlay)

    # Grid line color: slightly darker than grout, warm brown tint
    GRID_LINE_COLOR = (155, 138, 112, 25)  # warm brown, alpha 25

    # Horizontal rows — evenly spaced across floor height
    y = floor_top_y
    while y < H:
        gd.line([(0, y), (W, y)], fill=GRID_LINE_COLOR, width=2)
        y += TILE_SIZE

    # Vertical columns — evenly spaced across floor width
    x = 0
    while x < W:
        gd.line([(x, floor_top_y), (x, H)], fill=GRID_LINE_COLOR, width=2)
        x += TILE_SIZE
...
# ── v002 ADDITION: Upper Wall Wallpaper Texture ───────────────────────────────

def draw_upper_wall_texture(img, bw_left, bw_top, bw_right, bw_bot):
    """
    v002 ADDITION: Subtle period wallpaper texture on the upper wall area.
    Horizontal stripes at very low opacity (12–15%) alternating warm cream/buff
    with the base wall color. Very subtle — the wall should feel textured but
    not patterned. Evokes period linoleum-era kitchen wallpaper.

    Applied only to upper portion of back wall (above counter / above mid-wall),
    where the wall surface is most visually prominent and undifferentiated.
    """
    wall_height = bw_bot - bw_top

    # Upper wall zone: from top of back wall to about 55% down
    # (below this is cabinets and countertop, so wallpaper only visible up top)
    upper_wall_y1 = bw_top
    upper_wall_y2 = bw_top + int(wall_height * 0.50)

    # Stripe pattern: alternate between two very close warm tones
    # WALL_WARM base vs slightly warmer/cooler buff stripe
    STRIPE_A = (248, 238, 210)   # very pale warm cream (barely lighter than WALL_WARM)
    STRIPE_B = (232, 214, 178)   # slightly deeper warm buff (barely darker)
    STRIPE_HEIGHT = 12           # pixels per stripe (fine horizontal lines)
    STRIPE_ALPHA_A = 12          # very low opacity for A stripes
    STRIPE_ALPHA_B = 15          # slightly stronger for B stripes
    ...
    while y < upper_wall_y2:
        ...
        # Draw stripe across the back wall width
...
def main():
    ...
    print("LTG_TOOL_bg_grandma_kitchen_v002.py")
    print("Rendering Grandma Miri's Kitchen v002 — A1-01 (Act 1 opening)...")
    print("v002 improvements: floor tile grid, upper wall texture, enhanced CRT glow")

    print("  [1] Base room — walls, ceiling, floor...")
    draw, vp_x, vp_y, bw_left, bw_top, bw_right, bw_bot = draw_base_room(img)

    print("  [2] Floor tiles (linoleum, worn path)...")
    draw_floor_tiles(draw, bw_left, bw_bot, bw_right, vp_x, vp_y)
    ...
    print("  [4] Doorway + CRT TV in adjacent room (enhanced glow v002)...")
    ...
    print("  [10] v002: Upper wall wallpaper texture (horizontal stripes, low alpha)...")
    img = draw_upper_wall_texture(img, bw_left, bw_top, bw_right, bw_bot)
    draw = ImageDraw.Draw(img)

    print("  [11] v002: Floor linoleum grid overlay + worn path accent...")
    ...
    out_path = "/home/wipkat/team/output/backgrounds/environments/LTG_ENV_grandma_kitchen_v002.png"
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    img.save(out_path, "PNG")
    size_bytes = os.path.getsize(out_path)
    print(f"\nSaved: {out_path}")
    print(f"File size: {size_bytes:,} bytes ({size_bytes // 1024} KB)")
    print("\nSpec verification (v002):")
    ...
    print("  - CRT TV through doorway (story element — enhanced glow v002) ✓")
    ...
    print("  - v002: Floor linoleum tile grid visible (2px, alpha 25, 60×60px) ✓")
    print("  - v002: Worn path from doorway to stove zone (lighter rect, alpha 20) ✓")
    ...
    print("  - NOTE: ltg_render_lib.py not yet available; scanline_overlay() skipped ✓")
