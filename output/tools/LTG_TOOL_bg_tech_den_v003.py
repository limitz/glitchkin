#!/usr/bin/env python3
"""
LTG_TOOL_bg_tech_den_v003.py — Cosmo's Tech Den Background v003 (Daylight)
"Luma & the Glitchkin" — Background & Environment Design
Artist: Kai Nakamura | Cycle 21

Upgrade from v002: replaces all inline lighting code with calls to ltg_render_lib.
All visual content (desks, monitors, shelving, jacket, bed, posters) preserved.

Changes from v002:
  - light_shaft()    replaces draw_light_shaft() — window beam with feathered blur
  - dust_motes()     replaces draw_dust_motes()  — beam particles via shared lib
  - gaussian_glow()  replaces soft_glow_overlay() for monitor glow spill
  - vignette()       added as final compositing pass

Rules:
  - Real World palette ONLY — zero Glitch palette colors
  - After img.paste() / alpha_composite: refresh draw = ImageDraw.Draw(img)
  - Never overwrite outputs — versioned v003
  - All procedural elements use seeded RNG

Output: /home/wipkat/team/output/backgrounds/environments/LTG_ENV_tech_den_v003.png
"""
...
def draw_tech_den():
    ...
    # Shaft fans from window right-edge outward and down across desk/floor.
    SHAFT_APEX      = (WIN_X1 + 5,   mid_y - 50)
    SHAFT_BASE_LEFT = (WIN_X0,       FLOOR_Y_FAR + 140)
    SHAFT_BASE_RIGHT= (WIN_X1 + 330, FLOOR_Y_FAR + 110)

    light_shaft(img, SHAFT_APEX, SHAFT_BASE_LEFT, SHAFT_BASE_RIGHT,
                SUNLIT_AMBER, max_alpha=115)
    draw = ImageDraw.Draw(img)

    # ── DUST MOTES in beam via ltg_render_lib.dust_motes() ───────────────────
    # Bound the scatter to the beam bounding box
    ...
    # CRT1 glow spill onto wall / desk surface
    gaussian_glow(img, (CRT1_CX, DESK_TOP_Y - 100), 200, MON_GLOW_SOFT,
                  max_alpha=50, steps=8)
    draw = ImageDraw.Draw(img)

    # Wide monitor spill onto desk surface (elliptical, centered on desk zone)
    ...
    # Chair back spill — via gaussian_glow
    ...
    # ── OUTPUT ────────────────────────────────────────────────────────────────
    out_path = "/home/wipkat/team/output/backgrounds/environments/LTG_ENV_tech_den_v003.png"
