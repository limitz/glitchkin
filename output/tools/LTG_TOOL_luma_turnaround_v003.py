#!/usr/bin/env python3
"""
LTG_TOOL_luma_turnaround_v003.py
Luma — 4-View Character Turnaround v003
"Luma & the Glitchkin" — Cycle 28 / Maya Santos

v003 UPDATE (C28 — Line Weight Fix):
  - All line weights corrected to v006 standard (3-tier at 2× render):
      head outline → width=4
      structure (torso, arms, neck, ear) → width=3
      detail (pocket, laces, seams) → width=2
  - FRONT and 3/4 torso polygon: was width=5, now width=3
  - Head outlines (FRONT, 3/4, BACK): was width=6, now width=4
  - SIDE head polygon: was width=5, now width=4
  - SIDE torso edges: was width=5/4, now width=3
  - Cheek nub outline: was width=4, now width=3
  - Chin arc (FRONT): was width=6, now width=4
  - BACK head outline: was width=6, now width=4
  - BACK torso: was width=5, now width=3
  - BACK hood: was width=4, now width=3
  - 3.2 heads proportions preserved (canonical)
  - Act 2 design: A-line hoodie, chunky sneakers (unchanged)
  - BACK view confirmed present
  - Output thumbnail applied: 1600×700 → 1280×560 (≤ 1280px rule)
...
def build_turnaround():
    ...
    title = "LUMA — Character Turnaround v003  |  Luma & the Glitchkin  |  Cycle 28 / Maya Santos"
    sub   = "v003: Line weight fix — head=4, structure=3, detail=2 (v006 standard) | 3.2 heads"
...
def main():
    ...
    out_path = os.path.join(out_dir, "LTG_CHAR_luma_turnaround_v003.png")
    img.save(out_path)
    print(f"Saved: {out_path}")
    print(f"  Size: {img.size[0]}x{img.size[1]}px")
    print("  v003: Line weight fix (head=4, structure=3, detail=2) | 3.2 heads | 4 views")
