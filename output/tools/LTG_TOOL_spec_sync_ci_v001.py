...
Usage
-----
    python LTG_TOOL_spec_sync_ci_v001.py
    python LTG_TOOL_spec_sync_ci_v001.py --chars all
    python LTG_TOOL_spec_sync_ci_v001.py --chars luma cosmo
    python LTG_TOOL_spec_sync_ci_v001.py --chars glitch --tools-dir /path/to/tools
    python LTG_TOOL_spec_sync_ci_v001.py --json          # machine-readable output
...
API
---
    from LTG_TOOL_spec_sync_ci_v001 import run_ci, format_ci_report
...
def _run_char_spec_lint(char_name: str, tools_dir: str) -> Dict:
    ...
    lint_module_path = os.path.join(tools_dir, "LTG_TOOL_char_spec_lint_v001.py")
...
def _run_glitch_spec_lint(tools_dir: str) -> Dict:
    ...
    """
    lint_module_path = os.path.join(tools_dir, "LTG_TOOL_glitch_spec_lint_v001.py")
    if not os.path.isfile(lint_module_path):
        return {
            "char": "glitch",
            "overall": "ERROR",
...
if __name__ == "__main__":
    ...
    if args.json:
        ...
        if args.save_report:
            ...
            save_path = os.path.join(tools_dir, "LTG_TOOL_spec_sync_ci_v001_report.txt")
