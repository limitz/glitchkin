#!/usr/bin/env python3
"""
LTG_TOOL_cosmo_expression_sheet_v006.py
Cosmo Expression Sheet Generator — "Luma & the Glitchkin"

v006 — C37 P1 Fix: S003 glasses_tilt compliance (Maya Santos)

  KEY CHANGES from v005:
  - S003 COMPLIANCE FIX: glasses_tilt corrected to 7° on all expressions.
    - v005 had AWKWARD=10°, SURPRISED=10°, FRUSTRATED/DEFEATED=10° — all exceed
      spec maximum of 9° (7° ± 2° tolerance). SKEPTICAL was 9° (at tolerance edge,
      accepted). DETERMINED and WORRIED were already at 7°.
    - All expressions now use glasses_tilt ∈ {7, 8, 9} to remain within ±2° of 7°.
    - Specifically: AWKWARD 10→7, SURPRISED 10→7, FRUSTRATED/DEFEATED 10→7.
  - All other content (poses, palette, layout, face expressions) unchanged from v005.
  - Spec verified: max tilt = 9° (SKEPTICAL), target = 7°, all within ±2° tolerance.

  Silhouette differentiation from v005 is preserved:
...
Output: output/characters/main/LTG_CHAR_cosmo_expression_sheet_v006.png
Author: Maya Santos — Cycle 37 (S003 compliance fix)
Date: 2026-03-30
"""
...
def generate_cosmo_expression_sheet(output_path):
    ...
    draw.text((PAD, 14),
              "COSMO — Expression Sheet — Luma & the Glitchkin  |  v006  |  C37: S003 glasses_tilt fix (7deg)",
    ...
    print("v006 fix: S003 glasses_tilt — AWKWARD/SURPRISED/FRUSTRATED corrected to 7deg (was 10deg)")
    print("Spec compliance: all glasses_tilt in {7, 8, 9} = within 7deg +/- 2deg tolerance")
...
if __name__ == '__main__':
    ...
    generate_cosmo_expression_sheet(
        os.path.join(out_dir, "LTG_CHAR_cosmo_expression_sheet_v006.png")
    )
