...
"""
LTG_TOOL_char_luma.py — Canonical Luma Renderer Module v1.0.0
...
def _draw_arm_forward_reaching(ctx, ls_pt, head_cx, head_r, s, hoodie, lw_major, lw_minor, lw_accent, arm_w_top, arm_w_bot, hand_r_s):
    ...
    _draw_variable_stroke_limb(ctx, upper, arm_w_top, arm_w_bot * 1.1, hoodie, LINE_COL, lw_major)
    _draw_variable_stroke_limb(ctx, fore, arm_w_bot * 1.1, arm_w_bot, hoodie, LINE_COL, lw_major)
...
def draw_luma(expression, pose=None, scale=1.0, facing="right", scene_lighting=None):
    ...
    info = _draw_luma_on_context(ctx, cx, ground_y, char_h, expression, spec, scale)
...
if __name__ == "__main__":
    ...
    for expr in EXPRESSIONS:
        ...
        out_path = str(output_dir("characters", "main", f"LTG_CHAR_luma_{expr.lower()}_test.png"))
