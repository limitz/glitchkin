...
EXPRESSIONS = [
    ...
    # ── SKEPTICAL — LEAN FIX ────────────────────────────────────────────────────
    # body_tilt=6 now produces 6 * 2.5 = 15px displacement (was 2.4px — invisible).
    # Arms pulled tighter: arm_l_dy=-14, arm_r_dy=-10 (notebook hugged to chest).
    # Compound body-language: backward lean (torso) + contained arms = skeptical containment.
    {
        "name":     "SKEPTICAL",
        "body_data": {
            "arm_l_dy": -14, "arm_r_dy": -10,   # Cycle 19: arms tighter (was -8, -5)
...
def generate_cosmo_expression_sheet(output_path):
    ...
    print("Lean fix applied: tilt_off multiplier 0.4 → 2.5")
    print(f"  SKEPTICAL tilt=6: was 2.4px → now {int(6 * 2.5)}px displacement")
    print(f"  DETERMINED tilt=-5: was 2.0px → now {int(5 * 2.5)}px forward lean")
