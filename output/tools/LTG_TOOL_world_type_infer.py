...
Usage as module:
    from LTG_TOOL_world_type_infer import infer_world_type, get_warm_cool_threshold

    wt  = infer_world_type("LTG_COLOR_styleframe_discovery.png")  # → "REAL"
    thr = get_warm_cool_threshold(wt)                                   # → 12.0

Usage as CLI:
    python LTG_TOOL_world_type_infer.py path/to/asset.png
    python LTG_TOOL_world_type_infer.py --threshold path/to/asset.png
    python LTG_TOOL_world_type_infer.py --batch output/color/style_frames/
    python LTG_TOOL_world_type_infer.py --list-rules
...
Changelog:
  ...
  1.0.0 (Cycle 38): Initial implementation. Standalone extraction of the
                    infer_world_type() logic from LTG_TOOL_palette_warmth_lint.
...
def infer_world_type(filename_or_path: str) -> Optional[str]:
    ...
    filename_or_path : str
        ...
        Examples: "LTG_COLOR_styleframe_discovery.png",
                  "LTG_TOOL_bg_grandma_kitchen.py",
                  "/home/wipkat/team/output/color/style_frames/LTG_COLOR_styleframe_glitch_storm.png"
    ...
    Examples
    --------
    >>> infer_world_type("LTG_COLOR_styleframe_discovery.png")
    'REAL'
    >>> infer_world_type("LTG_TOOL_style_frame_03_other_side.py")
    'OTHER_SIDE'
    >>> infer_world_type("LTG_TOOL_bg_glitch_layer_encounter.py")
    'GLITCH'
    >>> infer_world_type("LTG_ENV_grandma_kitchen.png")
    'REAL'
    >>> infer_world_type("LTG_CHAR_luma_expressions.png")  # character sheet
    None
    >>> infer_world_type("LTG_BRAND_logo.png")
    None
    """
...
def infer_and_threshold(filename_or_path: str) -> Tuple[Optional[str], float]:
    ...
    Examples
    --------
    >>> infer_and_threshold("LTG_COLOR_styleframe_discovery.png")
    ('REAL', 12.0)
    >>> infer_and_threshold("LTG_COLOR_styleframe_otherside.png")
    ('OTHER_SIDE', 0.0)
    """
...
def _cli(argv: List[str]) -> int:
    ...
    Modes:
      python LTG_TOOL_world_type_infer.py <path> [<path> ...]
          Infer world type for one or more file paths.

      python LTG_TOOL_world_type_infer.py --threshold <path>
          Print only the float threshold to stdout (for shell capture).

      python LTG_TOOL_world_type_infer.py --batch <directory>
    ...
    """
    if not argv:
        print(f"LTG_TOOL_world_type_infer  v{__version__}")
        print("Usage:")
        print("  python LTG_TOOL_world_type_infer.py <path> [<path> ...]")
        print("  python LTG_TOOL_world_type_infer.py --threshold <path>")
        print("  python LTG_TOOL_world_type_infer.py --batch <directory>")
        print("  python LTG_TOOL_world_type_infer.py --list-rules")
        return 0
