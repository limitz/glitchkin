#!/usr/bin/env python3
"""
LTG_TOOL_style_frame_03_other_side_v003.py
"Luma & the Glitchkin" — Style Frame 03 "The Other Side" — Critique C9 Fix Pass

Artist: Jordan Reed | Cycle 19
Based on: LTG_TOOL_style_frame_03_other_side_v002.py (Jordan Reed, Cycle 16)

Cycle 19 fixes (Critique C9 feedback):
  1. BYTE BODY COLOR — Fixed CRITICAL bug: BYTE_BODY was (10,10,20) Void Black.
     Byte was INVISIBLE against UV Purple ambient background.
     Now BYTE_BODY = (0, 212, 232) — GL-01b Byte Teal (#00D4E8).
  2. EYE SIZE FIX — eye_r formula max(2, h//7) produced ~10px radius at h=75.
     Changed to max(15, h//5) — minimum 15px radius (30px diameter), both eyes readable.
  3. EYE FIX — Removed Void Black diagonal slash from magenta eye.
     Slashes on eyes obliterate readability against dark BG.
     Both cyan and magenta eyes now clear, unobstructed, readable.
...
Output: /home/wipkat/team/output/color/style_frames/LTG_COLOR_styleframe_otherside_v003.png
"""
...
def draw_footer(draw, img):
    ...
    label_items = [
        (12, "LTG — STYLE FRAME 03 — \"THE OTHER SIDE\" v003"),
        (480, "BYTE TEAL #00D4E8 | EYE FIX | VOID BLACK + UV_PURPLE | NO WARM LIGHT"),
        (960, "1920x1080 | JORDAN REED | CYCLE 19"),
        (1500, "LTG_COLOR_styleframe_otherside_v003"),
    ]
    try:
...
def generate(output_path):
    ...
    print("  [FIX 1] BYTE_BODY = (0,212,232) GL-01b Byte Teal — was (10,10,20) VOID BLACK ✓")
    print("  [FIX 2] Eye radius = max(15, h//5) — minimum 15px radius, both eyes readable ✓")
    print("  [FIX 3] Void Black diagonal slash REMOVED from magenta eye — clean read ✓")
    print("  [CARRY] DRW-18 UV Purple rim on Luma hair crown — from v002 ✓")
    print("  [CARRY] Waterfall luminance reduced — ambient flow not solid wall — from v002 ✓")
    print("  [CARRY] Mid-distance bridging element — from v002 ✓")
    print("  [CARRY] Right-side void irregularity — from v002 ✓")


if __name__ == "__main__":
    OUTPUT_PATH = "/home/wipkat/team/output/color/style_frames/LTG_COLOR_styleframe_otherside_v003.png"
