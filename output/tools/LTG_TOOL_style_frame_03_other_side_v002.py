#!/usr/bin/env python3
"""
LTG_TOOL_style_frame_03_other_side_v002.py
"Luma & the Glitchkin" — Style Frame 03 "The Other Side" — Cycle 16 Fix Pass

Artist: Jordan Reed | Cycle 16
Based on: LTG_TOOL_style_frame_03_other_side_v001.py (Jordan Reed, Cycle 15)

Cycle 16 fixes (Jordan Reed — Critique C8 / Takeshi + Naomi feedback):
  1. DATA WATERFALL LUMINANCE — Pulled down significantly. Waterfall was bisecting
     the composition like a wall. Now renders at reduced alpha/brightness —
     reads as ambient data flow, not a solid column. Width also reduced.
  2. MID-DISTANCE BRIDGING ELEMENT — Added a floating architectural fragment
     bridge at mid-distance to fill compositional gap between near platform
     and far void structures. Spans ~40-60% x zone, ~45-58% y zone.
  3. RIGHT-SIDE VOID UNIFORMITY — Right-side structures broken up with
     irregularity and scale variation. Far slabs now use seeded variation in
     position/scale/rotation-implied angle. No more gridded pattern.
  4. DRW-18 UV Purple rim on Luma's hair crown — #7B2FBE GL-04 UV Purple rim
     added as a distinct bright rim strip to Luma's hair crown so her head
     doesn't merge with dark background structures. Sam Kowalski color note.

CRITICAL RULES (unchanged from v001):
...
# DRW-18 UV Purple rim for Luma's hair crown (FIX 4)
...
def draw_far_distance(draw, img):
    """
    FIX 3: Right-side void structures broken up with irregularity and scale variation.
    Far slabs use seeded variation. No more gridded pattern.
    """
    haze_top = 0
    haze_bot = int(H * 0.45)
    ...
    # Far slabs — left/center (unchanged from v001)
    left_slab_specs = [
        (0.20, 0.08, 0.50, 0.18),
        (0.55, 0.12, 0.85, 0.24),
        (0.62, 0.20, 0.88, 0.28),
    ...
    # FIX 3: Right-side void structures with irregularity and scale variation
    # Use seeded rng for reproducibility. Varied sizes, positions, angles (implied via polygons)
    rng_rs = random.Random(73)
    right_slab_base = [
        # (cx_frac, cy_frac, base_w_frac, base_h_frac)
        (0.72, 0.28, 0.12, 0.09),
        (0.80, 0.14, 0.08, 0.06),
        (0.88, 0.22, 0.14, 0.07),
        (0.76, 0.36, 0.06, 0.10),
    ...
    for i, (cxf, cyf, wf, hf) in enumerate(right_slab_base):
        # Apply seeded scale variation (+/- 30%)
        ...
        # Use polygon with slight skew to break up rectangular grid appearance
        skew = rng_rs.randint(-8, 8)
        poly = [(x1 + skew, y1), (x2 + skew, y1), (x2, y2), (x1, y2)]
        draw.polygon(poly, fill=FAR_EDGE, outline=UV_PURPLE)

        # Some slabs get secondary detail — tiny inner fragment for depth
    ...
    # Distant data waterfalls — thin suggestion lines (unchanged)
...
def draw_mid_distance(draw, img):
    """
    FIX 2: Added mid-distance bridging element (floating arch fragment) to
    fill compositional gap between near platform and far void structures.
    """
    # Geometric slabs (unchanged from v001)
    mid_slabs = [
    ...
    # FIX 2: Mid-distance bridging element — floating arch fragment
    # Fills compositional gap between near platform (left) and far void (right-up)
    # Position: ~40-65% x, ~48-60% y — bridges the empty mid-distance band
    ...
    # Hanging pillar fragments — suggest arch/architectural remnants
    pillar_specs = [
    ...
    # Secondary smaller floating fragment above bridge — adds depth layering
    frag_x1 = int(W * 0.50)
    frag_x2 = int(W * 0.58)
    frag_y1 = int(H * 0.43)
    frag_y2 = int(H * 0.46)
...
def draw_midground(draw, img):
    """
    FIX 1: Data waterfall luminance pulled down significantly.
    Was bisecting composition like a wall. Now reads as ambient data flow.
    Width reduced from 4px to 2px range. BASE alpha reduced ~50%.
    """
    plat_x1 = 0
    plat_y1 = int(H * 0.66)
    plat_x2 = int(W * 0.45)
    ...
    # ── FIX 1: Data Waterfall — reduced luminance and width ──────────────────
    # Was: full-alpha opaque column of DATA_BLUE_90 = wall effect
    # Now: narrower, lower alpha, reads as ambient data flow not solid barrier
    wf_x1 = int(W * 0.39)   # narrowed (was 0.38)
    wf_x2 = int(W * 0.41)   # narrowed (was 0.42) — 2px range not 4px
    wf_y_top = int(H * 0.18)
    wf_y_bot = int(H * 0.66)

    # Reduced base fill — alpha-composite at 45% instead of per-pixel solid draw
    ...
    for x in range(wf_x1, wf_x2):
        ...
        # FIX 1: alpha was ~255 per pixel; now max 110 with edge falloff
    ...
    # Code character highlights — sparse, reduced from 35 to 18 particles
    ...
    # Data mist at base — gradient DATA_BLUE → UV_PURPLE (reduced alpha)
    mist_y1 = int(H * 0.62)
    mist_y2 = int(H * 0.68)
    ...
    # Light pool on platform — soft, unchanged
    ...
    for r in range(60, 0, -1):
        t = 1.0 - r / 60.0
        alpha = int(t * t * 25)  # reduced from 35 to 25
...
def draw_lighting_overlay(img):
    ...
    FIX 1: DATA_BLUE waterfall strip reduced to match reduced waterfall luminance.
    """
    ...
    # FIX 1: Waterfall light strip — reduced alpha (was 35, now 18)
    wf_x1 = int(W * 0.36)
    wf_x2 = int(W * 0.44)
    for x in range(wf_x1, wf_x2):
        dist_from_center = abs(x - (wf_x1 + wf_x2) // 2)
        ...
        alpha = int(t * 18)  # reduced from 35 to 18
...
def draw_luma(draw, cx, cy, h):
    ...
    FIX 4: DRW-18 UV Purple rim (#7B2FBE) added to Luma's hair crown.
    Prevents head from merging with dark background structures.
    """
    head_h = h // 5
    ...
    # UV Purple sheen (general)
    sheen_h = hair_h // 2
    draw.rectangle([head_x + 1, head_top - hair_h + 2,
                    head_x + head_w - 2, head_top - hair_h + 2 + sheen_h],
                   fill=HAIR_UV_SHEEN)

    # FIX 4: DRW-18 UV Purple RIM on hair crown — #7B2FBE bright rim
    # This is a distinct bright edge strip to prevent Luma's head merging with
    # dark background structures. Sam Kowalski confirmed missing in v001.
...
def draw_byte(draw, cx, cy, h):
    """
    Draw Byte — ELEC_CYAN outline (home territory = more alive).
...
def draw_footer(draw, img):
    ...
    label_items = [
        (12, "LTG — STYLE FRAME 03 — \"THE OTHER SIDE\" v002"),
        (480, "VOID + ELEC_CYAN + UV_PURPLE | NO WARM LIGHT"),
        (960, "1920x1080 | JORDAN REED | CYCLE 16"),
        (1500, "LTG_COLOR_styleframe_otherside_v002"),
    ]
    try:
...
def generate(output_path):
    ...
    print("Layer 4: Far distance (FIX 3: right-side irregularity)...")
    draw = draw_far_distance(draw, img)

    print("Layer 3: Mid-distance structures (FIX 2: bridging element added)...")
    draw = draw_mid_distance(draw, img)

    print("Layer 2: Midground (FIX 1: waterfall luminance reduced)...")
    draw = draw_midground(draw, img)

    print("Layer 1: Foreground (settled confetti)...")
    draw = draw_foreground(draw)

    print("Lighting overlay (FIX 1: waterfall strip alpha reduced)...")
    draw = draw_lighting_overlay(img)

    print("Character: Luma (FIX 4: DRW-18 UV Purple hair rim)...")
    luma_cx = int(W * 0.14)
    luma_cy = int(H * 0.66)
    luma_h  = int(H * 0.20)
    ...
    print("\nFix verification:")
    print("  [FIX 1] Waterfall: alpha max 110 (was 255), strip alpha 18 (was 35) — ambient flow ✓")
    print("  [FIX 2] Mid-distance bridge: floating arch fragment at 40-65% x / 49-60% y ✓")
    print("  [FIX 3] Right-side void: 7 slabs with seeded scale variation + polygon skew ✓")
    print("  [FIX 4] Luma hair: HAIR_UV_RIM (#7B2FBE) 2px bright rim on crown ✓")


if __name__ == "__main__":
    OUTPUT_PATH = "/home/wipkat/team/output/color/style_frames/LTG_COLOR_styleframe_otherside_v002.png"
