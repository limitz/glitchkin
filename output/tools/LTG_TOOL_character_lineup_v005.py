#!/usr/bin/env python3
"""
LTG_TOOL_character_lineup_v005.py
Character Lineup Generator — Luma & the Glitchkin
Cycle 27: Updated Luma to v006-era construction.
...
Cycle 27 changes (Maya Santos, Character Designer):
  - Luma draw_luma_lineup completely rebuilt to match v006 canonical style:
      * Hair: 8 overlapping ellipses (curl cloud) scaled from classroom reference
        using s = r / 100.0. Replaces the 1-ellipse mass from v004.
      * Head: circle + lower chin fill + CHEEK NUBS at sides.
        Replaces the bare single ellipse from v004.
      * Eyes: near-circular ovals with iris, pupil, highlight, eyelid arc.
        Replaces the tiny symmetric circles from v004.
      * Nose: two nostril dots + bridge arc (detail weight width=2).
      * Mouth: neutral slight-smile bezier arc (structure weight width=2).
      * Three-tier line weight at lineup scale (r≈37px):
          Head outline: width=2  (proportional to head=4 at HR=104 scale)
          Structure:    width=2  (proportional to structure=3 at HR=104 scale)
          Detail:       width=1  (proportional to detail=2 at HR=104 scale)
      * Body (hoodie, legs, shoes) retained from v004 — unchanged.
  - Other characters (Byte, Cosmo, Miri, Glitch) unchanged from v004.
  - Title updated to Cycle 27.
  - Output: LTG_CHAR_lineup_v005.png (never overwrites v004).
...
Output: /home/wipkat/team/output/characters/main/LTG_CHAR_lineup_v005.png
Usage: python3 LTG_TOOL_character_lineup_v005.py
"""
...
def draw_luma_lineup(draw, cx, base_y, h):
    """Luma full body at lineup height h, using v006 canonical construction.
    ...
    """
    hu = h / 3.5
    r  = int(hu * 0.46)     # head radius ≈ 37px
...
def generate_lineup(output_path):
    ...
    title = "LUMA & THE GLITCHKIN — Full Cast Height Lineup — Cycle 27 (Luma v006 style)"
    ...
    footer = (
        ...
        "Colors per master_palette.md (canonical). Cycle 27 — Luma updated to v006 style."
    )
...
def main():
    ...
    generate_lineup(os.path.join(out_dir, "LTG_CHAR_lineup_v005.png"))
    print("Character lineup v005 generation complete.")
    print("  Cast: Luma (v006 style), Byte, Cosmo, Miri, Glitch (5 characters)")
