...
def run_warmth_hook(out_path: str, enabled: bool = True) -> str | None:
    ...
    inject_module_path = os.path.join(tools_dir, "LTG_TOOL_warmth_inject_v001.py")

    if not os.path.isfile(inject_module_path):
        print(
            f"[warmth_hook] ERROR: LTG_TOOL_warmth_inject_v001.py not found in {tools_dir}",
            file=sys.stderr,
        )
        return None
    ...
    spec = importlib.util.spec_from_file_location("LTG_TOOL_warmth_inject_v001", inject_module_path)
