...
def _load_header_h(family="luma", default=54):
    """Load HEADER_H for this family from sheet_geometry_config.json."""
    try:
        import json
        with open(_CONFIG_PATH) as f:
            cfg = json.load(f)
        geo = cfg.get("families", {}).get(family, {})
        # For luma: panel_top_abs is the effective HEADER_H (title bar + pad)
...
COLS, ROWS = 4, 1
PAD = 14
# C40: TITLE_H loaded from sheet_geometry_config.json via _LUMA_PANEL_TOP.
# _LUMA_PANEL_TOP is the absolute y where panels begin = PAD + TITLE_H_PIXELS.
# Back out the raw title bar height for PANEL_H calculation:
_TITLE_H = max(_LUMA_PANEL_TOP - PAD, 40)  # minimum 40px
...
def panel_origin(col):
    ...
    y = _LUMA_PANEL_TOP   # C40: from sheet_geometry_config.json
    return x, y
