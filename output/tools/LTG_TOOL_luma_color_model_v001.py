...
All palette values taken from canonical sources:
  luma_color_model.md (color_models dir)

Output: output/characters/color_models/LTG_COLOR_luma_color_model_v001.png

[Renamed from LTG_COLOR_luma_color_model_v001.py to LTG_TOOL_luma_color_model_v001.py
...
def draw_character_silhouette(draw, img, cx, cy):
    ...
    for side in [-1, 1]:
        ...
        draw.ellipse([ex - 6, ey - eye_r_y + 2,
                      ex - 2, ey - eye_r_y + 6], fill=EYE_HL)
    # Nose
    ...
    for side in [-1, 1]:
        ...
        draw.arc([bx - 14, by - 4, bx + 14, by + 6],
...
def build_color_model():
    ...
    title = "LUMA — Color Model v001  |  Cycle 25  |  HOODIE ORANGE #E8703A PRIMARY"
    try:
...
def main():
    ...
    out_path = os.path.join(out_dir, "LTG_COLOR_luma_color_model_v001.png")
    model.save(out_path)
