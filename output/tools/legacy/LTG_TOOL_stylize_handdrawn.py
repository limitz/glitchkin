...
__cycle__ = 28  # C28: Added _restore_canonical_colors() to undo channel-separation hue drift at canonical color boundaries
...
CANONICAL_PALETTE = {
    "CORRUPT_AMBER":  {"rgb": (255, 140,   0), "hex": "#FF8C00"},   # GL-07  PIL H ~23
    "BYTE_TEAL":      {"rgb": (  0, 212, 232), "hex": "#00D4E8"},   # GL-01b PIL H ~131
    "UV_PURPLE":      {"rgb": (123,  47, 190), "hex": "#7B2FBE"},   # GL-04  PIL H ~193  (FIXED C27: was #6A0DAD)
    "HOT_MAGENTA":    {"rgb": (255,  45, 107), "hex": "#FF2D6B"},   # GL-06  PIL H ~240  (FIXED C27: was #FF0080)
    "ELECTRIC_CYAN":  {"rgb": (  0, 240, 255), "hex": "#00F0FF"},   # GL-01a PIL H ~130
    "SUNLIT_AMBER":   {"rgb": (212, 146,  58), "hex": "#D4923A"},   # RW-03  PIL H ~24
}
...
def _apply_glitch_treatment(img, intensity=1.0, seed=42):
    ...
    Notes:
      - NO paper grain in Glitch Layer.
      - NO warm color bleed in Glitch Layer (GUARD: _pass_color_bleed() must
        NEVER be called here — SF03 Other Side must have zero SUNLIT_AMBER).
        Any SUNLIT_AMBER pixels appearing in glitch output are a bleed gate
        failure; they are caught and corrected by _restore_canonical_colors().
    """
    if img.mode != "RGBA":
        img = img.convert("RGBA")

    # WARM BLEED GUARD: _pass_color_bleed() is intentionally NOT called here.
    # Glitch mode images (e.g. SF03 Other Side) must contain zero warm tones.
    # If SUNLIT_AMBER appears in glitch output but not in source, it indicates
    # channel-separation bleed at warm-colored boundaries — handled by
    # _restore_canonical_colors() in stylize().
...
# ---------------------------------------------------------------------------
# Canonical color restore post-pass (C28)
# ---------------------------------------------------------------------------

def _restore_canonical_colors(original_img, processed_img, palette_dict, radius=25):
    """Restore pixels near canonical colors back to exact canonical values.

    After all processing passes (including geometric passes like channel
    separation that can cause hue drift at canonical color region boundaries),
    restore any pixel that was within `radius` Euclidean RGB distance of a
    canonical color in the ORIGINAL image to the EXACT canonical RGB value.

    This undoes drift introduced by _pass_color_separation() and similar
    geometric passes on canonical-colored regions (UV_PURPLE is especially
    vulnerable: high B, low R means channel offset creates warm-looking edges).

    Args:
        original_img  (PIL.Image): The original pre-processing image (RGB or RGBA).
        processed_img (PIL.Image): The processed image to restore into (RGBA).
        palette_dict  (dict): CANONICAL_PALETTE dict: {name: {"rgb": (r,g,b), ...}}
        radius        (int): Euclidean RGB distance threshold. Default 25.

    Returns:
        PIL.Image: processed_img with canonical pixels restored (RGBA).
    """
    try:
        import numpy as np

        orig_arr = np.array(original_img.convert('RGB'))
        # Ensure processed is RGBA
        if processed_img.mode != 'RGBA':
            processed_img = processed_img.convert('RGBA')
        proc_arr = np.array(processed_img).copy()

        for name, info in palette_dict.items():
            cr, cg, cb = info["rgb"]
            diff = orig_arr.astype(np.int32) - np.array([cr, cg, cb], dtype=np.int32)
        ...
        restored = Image.fromarray(proc_arr, 'RGBA')
        return restored

    except ImportError:
        # NumPy not available — fall back to pixel-by-pixel (slow for large images)
        if original_img.mode != 'RGB':
            original_img = original_img.convert('RGB')
        if processed_img.mode != 'RGBA':
            processed_img = processed_img.convert('RGBA')

        orig_px = original_img.load()
        proc_px = processed_img.load()
        w, h = original_img.size

        canonical_list = [(info["rgb"]) for info in palette_dict.values()]
        canonical_pairs = list(zip(palette_dict.keys(), canonical_list))

        for y in range(h):
            for x in range(w):
                r, g, b = orig_px[x, y][:3]
                for name, (cr, cg, cb) in canonical_pairs:
                    dist = ((r - cr) ** 2 + (g - cg) ** 2 + (b - cb) ** 2) ** 0.5
                    if dist <= radius:
                        a = proc_px[x, y][3]
                        proc_px[x, y] = (cr, cg, cb, a)
                        break
...
def stylize(
    ...
    # Keep a copy of the original for canonical color restore (C28)
    ...
    print(f"  Fixes active: [1] Full hue guard, [2] Chalk exclusions, "
          f"[3] Warm bleed gate, [4] Mixed mode cross-dissolve, [5] Canonical color restore (C28)")
    ...
    # C28 Fix 5: Restore canonical colors after all passes (undoes channel-separation drift)
    print(f"  Restoring canonical colors (radius=25)...")
    result = _restore_canonical_colors(original_img, result, CANONICAL_PALETTE, radius=25)
