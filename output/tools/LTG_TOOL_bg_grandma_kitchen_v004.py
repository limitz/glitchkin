#!/usr/bin/env python3
"""
LTG_TOOL_bg_grandma_kitchen_v004.py — Grandma Miri's Kitchen Background v004
"Luma & the Glitchkin" — Background & Environment Design
Artist: Jordan Reed | Cycle 35
...
v004 Rebuild (Chiara Ferrara C14 feedback, score 58/100):

  PROBLEM 1 — Value floor at 62 (spec requires ≤30):
    → FIX: deep_shadows pass adds DEEP_COCOA (#3B2820) and near-black fills to:
      - ceiling corners (top-left, top-right)
      - cabinet undersides (shadow below each upper cabinet)
      - under-counter zone (dark band below countertop)
      - floor corners and edges
      - table underside shadow
      - doorway interior (deep shadow in adjacent room)
    → All shadow zones push toward value ≤30

  PROBLEM 2 — Warm/cool separation 1.7 PIL units (spec requires ≥20):
    → FIX: Dual-temperature system strengthened:
      LEFT: SUNLIT_AMBER (#D4923A) warm column gradient from window, alpha max 80
            Direct window shaft as trapezoid of warm gold (not just ambient)
            Floor pool under window with near-255 hotspot at center
      RIGHT: CRT_COOL_SPILL = (0, 140, 155) desaturated electric cyan
             Cast from doorway (right side of left wall) into kitchen
             Cool light column from doorway threshold to right half of room
             Alpha max 65 — strong enough to pull QA separation ≥20

  PROBLEM 3 — Spatial coherence / "specific grandmother's kitchen":
    → FIX: Grandma Miri-specific details added:
      - Favorite mug with rose pattern (MIRI's documented color: WARM/WELCOMING)
      - Handwritten calendar on wall near stove
      - Knitting bag on chair (Miri is a knitter — character detail)
      - Worn apron hanging on peg by doorway
      - Medicine/supplement bottles on counter (elderly living detail)
      - Fridge magnets are travel destinations (world-experienced grandmother)
      - Half-finished crossword puzzle (daily ritual)

  Canvas: 1280×720 (direct, no thumbnail needed — already within limit)

All v003 improvements retained:
  - Side wall stripe texture
  - Perspective-correct linoleum floor grid
  - CRT dual-ring glow
  - Morning light shaft from window
  - Worn floor path

Output: /home/wipkat/team/output/backgrounds/environments/LTG_ENV_grandma_kitchen_v004.png
"""
...
def draw_temperature_split(img, win_y1, bw_bot):
    ...
    The QA tool (LTG_TOOL_render_qa_v001) measures warm/cool by comparing the
...
def main():
    ...
    print("LTG_TOOL_bg_grandma_kitchen_v004.py")
    print("Rendering Grandma Miri's Kitchen v004 — A1-01 (Act 1 opening)...")
    print("v004 fixes (Chiara Ferrara C14, score 58/100):")
    print("  Fix 1: Deep shadows (value floor ≤30) — DEEP_COCOA in corners/under cabinets")
    print("  Fix 2: Warm/cool separation ≥20 — warm SUNLIT_AMBER LEFT, cool CRT RIGHT")
    print("  Fix 3: Miri-specific spatial details (calendar, mug, knitting, apron, magnets)")
    print("  Canvas: 1280×720 (direct — no thumbnail needed)")
    ...
    print("  [8] Refrigerator (travel magnets — Miri detail)...")
    ...
    out_path = os.path.join(out_dir, "LTG_ENV_grandma_kitchen_v004.png")
    ...
    print("\nFix verification (C35 v004):")
    print("  [FIX v004-1] Deep shadows: DEEP_COCOA/NEAR_BLACK_WARM in ceiling corners,")
    print("               cabinet undersides, floor corners, table underside")
    print("               → Value floor should be ≤30 (was 62 in v003)")
    print("  [FIX v004-2] Warm/cool separation: SUNLIT_AMBER alpha-max=80 LEFT column,")
    print("               CRT_COOL_SPILL alpha-max=65 RIGHT from doorway")
    print("               → Warm/cool separation should be ≥20 (was 1.7 in v003)")
    print("  [FIX v004-3] Miri-specific details: rose mug, knitting bag, apron,")
    print("               travel magnets, medicine bottles, handwritten calendar")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="LTG_TOOL_bg_grandma_kitchen_v004 — Grandma Miri's Kitchen")
    parser.add_argument(
        "--check-warmth",
        action="store_true",
    ...
    main()
    out_path = "/home/wipkat/team/output/backgrounds/environments/LTG_ENV_grandma_kitchen_v004.png"
