...
Act 2 Contact Sheet v004 — Cycle 17
Lee Tanaka, Storyboard Artist

Changes from v003:
  - Adds A2-01 (Tech Den Wide — Act 2 opener, NEW)
  - Adds A2-05 (Millbrook Street exterior — walk-and-talk, NEW)
  - Adds A2-08 (Grandma Miri Returns — emotional climax, NEW)
  - Total: 11 panels (A1-04 + A2-01 through A2-08)
  - 3-row layout: 4 / 4 / 3 panels per row
  - Arc labels and border colors per arc segment
  - Version string: v004 / Cycle 17

Panel sequence (arc order):
  A1-04  — NEAR-MISS (classroom near-miss micro-beat)
  A2-01  — ESTABLISHED (Tech Den wide, Act 2 opener) [NEW]
  ...
  A2-05  — WALK+TALK (Millbrook exterior, Luma pitching) [NEW]
  ...
  A2-08  — RECOGNITION (Grandma Miri returns, emotional climax) [NEW]

Emotional arc:
  NEAR-MISS → ESTABLISHED → VULNERABLE → SKEPTICAL → INVESTIGATING →
  WALK+TALK → DETERMINED → HOPEFUL → FAILURE → RESIGNED → RECOGNITION

Output:
  /home/wipkat/team/output/storyboards/act2/LTG_SB_act2_contact_sheet_v004.png
"""
...
OUTPUT_PATH = os.path.join(ACT2_DIR, "LTG_SB_act2_contact_sheet_v004.png")
...
PANEL_SEQUENCE = [
    ...
    (PANELS_DIR + "/LTG_SB_act2_panel_a205_v001.png",
    ...
    (PANELS_DIR + "/LTG_SB_act2_panel_a208_v001.png",
     "A2-08\nrecognition",  "RECOGNITION",  (200, 180, 240)),
]

# Layout: 3 rows — 4 / 4 / 3 panels
ROW_LAYOUT = [4, 4, 3]
...
LABEL_H  = 28
HEADER_H = 42
ARC_LBL_H = 18   # height above thumbnail for arc label
...
def generate():
    ...
    sd.text((sheet_w - 155, 6), "v004 — Cycle 17",
            font=font_ann, fill=(150, 140, 110))
    # Arc summary
    arc_text = ("ARC: NEAR-MISS → ESTABLISHED → VULNERABLE → SKEPTICAL → "
                "INVESTIGATING → WALK+TALK → DETERMINED → HOPEFUL → FAILURE → RESIGNED → RECOGNITION")
    ...
    for row_idx, row_count in enumerate(ROW_LAYOUT):
        ...
        for col in range(row_count):
            ...
            if col < row_count - 1:
                ax  = x + THUMB_W + GUTTER // 2 + row_offset_x * 0
                # Actually connect from edge of this thumb to next
                gap_cx = x + THUMB_W + 2
                gap_cy = y + THUMB_H // 2
        ...
        if row_idx < n_rows - 1:
            # Arrow at right edge pointing down
    ...
    sd.text((MARGIN, footer_y),
            "v004 | Cycle 17 | 11 panels | A2-01 NEW + A2-05 NEW + A2-08 NEW | Full Act 2 arc",
...
if __name__ == "__main__":
    generate()
    print("Act 2 contact sheet v004 generation complete (Cycle 17).")
