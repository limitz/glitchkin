#!/usr/bin/env python3
"""
LTG_TOOL_character_lineup_v003.py
Character Lineup Generator — Luma & the Glitchkin
Cycle 14: Byte float-gap dimension arrow (Dmitri Volkov P1 — resolved)

Cycle 14 changes (Alex Chen, Art Director):
  - REPLACED Cycle 12 "ground floor." caption with a proper engineering dimension arrow.
    - Two-headed vertical arrow spanning Byte's bottom to the ground line.
    - Standard engineering style: short horizontal serif at each tip.
    - Label "0.25 HU" to the right of the arrow shaft.
    - Arrow color: GROUNDFLOOR_COL (100,168,200) — consistent with Cycle 12 annotation color.
    - Arrow positioned to the right of Byte's body (byte_cx + body_rx + 10).
    - Dashed ground-floor call-out line retained for context; "ground floor." text label removed.
  - Title updated to "Cycle 14".
  - Output: LTG_CHAR_lineup_v003.png (never overwrites v002).

Prior cycle history:
  Cycle 12 (Alex Chen): Ground-floor annotation added under Byte. Output: LTG_CHAR_lineup_v002.png.
  Cycle 10 (Alex Chen): Initial four-character lineup with color. Output: LTG_CHAR_lineup_v001.png.

NOTE: legacy source file is character_lineup_generator.py (pre-LTG naming convention).
Output: /home/wipkat/team/output/characters/main/LTG_CHAR_lineup_v003.png
Usage: python3 LTG_TOOL_character_lineup_v003.py
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# BYTE FLOAT-GAP DIMENSION ARROW
# ══════════════════════════════════════════════════════════════════════════════

def draw_byte_float_dimension(draw, font_small):
    """
    Draw a proper engineering dimension arrow for Byte's float gap.

    Cycle 14: Replaces the Cycle 12 caption-style "ground floor." annotation.
    Engineering dimension arrow style:
      - Vertical shaft between Byte's bottom (BASELINE_Y - float_gap) and ground (BASELINE_Y)
      - Two-headed: arrowhead at each end pointing toward the measured surface
      - Short horizontal serif (extension tick) at each tip — standard engineering notation
      - Label "0.25 HU" to the right of the shaft
      - Arrow positioned just to the right of Byte's body silhouette

    Geometry (derived from draw_byte_lineup constants):
      BYTE_H     = int(LUMA_RENDER_H * 0.58)  = ~162px
      float_gap  = int(BYTE_H * 0.18)          = ~29px
      body_rx    = BYTE_H // 2                  = ~81px

    The float-gap in the production bible is specified as 0.25 HU.
    HEAD_UNIT = LUMA_RENDER_H / LUMA_HEADS = 280 / 3.5 = 80px
    0.25 HU = 20px (canonical specification).
    The rendered float_gap (~29px) is the actual pixel value used in the script.
    The label "0.25 HU" documents the production bible spec (not the pixel count).
    """
    GROUNDFLOOR_COL = (100, 168, 200)   # same annotation color as Cycle 12
    ...
    # Arrow X position: just to the right of Byte's body (right edge + 10px margin)
    arrow_x  = byte_cx + body_rx + 10

    # Y positions
    top_y    = BASELINE_Y - float_gap   # Byte's bottom (bottom of oval + legs)
    bot_y    = BASELINE_Y               # ground line

    # Clamp arrow to be at least 6px tall to be legible
    if bot_y - top_y < 6:
        return
    ...
    # ── Arrowhead at TOP (pointing upward, toward Byte's bottom) ──────────────
    # Triangle tip at top_y, base below
    tip_size = 5
    draw.polygon(
        [(arrow_x,             top_y),
    ...
    # ── Arrowhead at BOTTOM (pointing downward, toward ground line) ───────────
    draw.polygon(
        [(arrow_x,             bot_y),
    ...
    # ── Serif extension ticks at each tip (horizontal, standard engineering) ──
    serif_w = 7   # half-width of the serif bar
    ...
    label_y  = (top_y + bot_y) // 2 - 5   # vertically centered on the gap
    draw.text((label_x, label_y), label, fill=GROUNDFLOOR_COL, font=font_small)

    # ── Dashed ground-floor call-out line (retained for context) ─────────────
    # A short dashed local line at BASELINE_Y under Byte's column to visually
    # anchor the bottom tick of the dimension arrow to the shared baseline.
    gf_x0 = byte_cx - 70
    gf_x1 = byte_cx + 70
    gf_y  = BASELINE_Y
    x = gf_x0
...
def generate_lineup(output_path):
    ...
    # Title
    title = "LUMA & THE GLITCHKIN — Character Height Lineup — Cycle 14"
    ...
    # ── Cycle 14: Byte float-gap dimension arrow (Dmitri Volkov P1 — resolved) ──
    # Replaces Cycle 12 "ground floor." caption with a proper engineering dimension arrow.
    ...
    footer = (
        ...
        "Colors per master_palette.md (canonical). Cycle 14 — Byte float-gap dimension arrow."
    )
...
def main():
    ...
    # Cycle 14: new versioned output — never overwrite existing assets
    generate_lineup(os.path.join(out_dir, "LTG_CHAR_lineup_v003.png"))
