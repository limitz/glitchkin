...
5. **FLAG 05**: OPEN C44. MIRI-A chopstick hair accessory is cross-cultural design error (Igbo-Nigerian/Brazilian heritage). Story recommendation (C43) and Maya Santos design assessment (C44) both confirm: replace with dark wooden hairpins. PENDING Alex Chen confirmation. Maya's impact assessment in Alex's inbox; Alex has not yet confirmed. Story team doc: LTG_miri_cultural_identity_brief.md. Jordan Reed notified to hold on accessory detail until Alex confirms.
...
- C44: Relationship style frame briefs for visual artists need: (1) exact scene and beat specified, not just "a warm moment"; (2) sightline asymmetry — which character is watching the other and why that is the frame's argument; (3) explicit CRT function in the frame (light source, background accent, foreground object) so the color architecture is unambiguous; (4) a one-line logline the artist can test the frame against. The ENV generator (Hana's kitchen v006) is canonical — brief the artist to place characters into the existing space, not to redesign it. Always note open design flags (FLAG 05) so the artist knows to hold on ambiguous details.
