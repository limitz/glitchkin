...
- **Check 11: dep_availability** — verifies bezier, Shapely, pycairo importable. WARN if missing. `check_dep_availability()` exported.
- **Check 12: bezier_migration_lint** — wraps curve_utils audit. WARN for pending migrations. `check_bezier_migration()` exported.
- **Check 13: tool_naming_lint** — enforces LTG_TOOL_ prefix on .py files. Legacy/deprecated exempt. `check_tool_naming()` exported.
- Registry updated: 13 slots in ci_check_registry.json.
- Runner map updated for all 13 checks.

### P2: Bezier Migration Batch 1 — 4 Files
- **LTG_TOOL_face_curve_validator.py** — FULL migration (local defs removed, imports added).
- **LTG_TOOL_luma_face_curves.py** — delegate wrappers (n= kwarg preserved).
- **LTG_TOOL_character_lineup.py** — delegate wrappers (_bezier3 + _polyline).
- **LTG_TOOL_luma_expression_sheet.py** — delegate wrappers (bezier3 + polyline).
- Post-migration audit: 1 MIGRATED, 3 PARTIAL, 5 NOT_MIGRATED (4 remaining + 1 exempt).

### P3: precritique_qa — No Action Needed
- Kai Nakamura already upgraded to v3.0.0 with sections 15-17, CYCLE_LABEL=C52.

### Deliverables
- ci_suite v2.0.0 (from v1.9.0): 3 new checks
- ci_check_registry.json updated (13 slots)
- 4 files bezier-migrated
- Report sent to Alex Chen inbox
- Archived 1 inbox message (C52 assignment)
- Ideabox: bezier_migration_batch_2 idea submitted

### Lessons Learned
- Thin delegate wrappers are a valid migration pattern when callers use different parameter names (n= vs steps=). The audit correctly detects these as PARTIAL since the local `def` still exists, but the math delegates to curve_utils. Full removal requires changing all call sites.
- The tool_naming_lint found non-compliant files (run_c31_qa.py, test_face_lighting.py) that predate the naming convention. These are known legacy scripts.

## CI Suite Status (C52)
- ci_suite v2.0.0 — 3 new checks added
- 13 checks total (was 10)
- Checks: stub_linter, draw_order_lint, glitch_spec_lint, spec_sync_ci, char_spec_lint, dual_output_check, hardcoded_path_check, thumbnail_lint, motion_sheet_coverage, doc_staleness, dep_availability, bezier_migration_lint, tool_naming_lint
- Checks loaded from ci_check_registry.json (config-driven)

## Bezier Migration Status (C52)
- 1 MIGRATED: face_curve_validator
- 3 PARTIAL: luma_face_curves, character_lineup, luma_expression_sheet
...
- 1 EXEMPT: rendering_comparison (cairo, not PIL)

## C51 Work Done

### P1: LTG_TOOL_curve_utils.py v1.0.0 (NEW)
- Shared bezier curve utilities wrapping `bezier` library + Shapely.
- **Section 1:** Drop-in replacements for hand-rolled bezier functions across 9 files.
  - `quadratic_bezier_pts()`, `cubic_bezier_pts()`, `cubic_bezier_single()`, `draw_bezier_polyline()`, `polyline()`.
  - Graceful fallback to hand-rolled math if bezier lib not installed.
- **Section 2:** Advanced ops (bezier library required).
  - `arc_length()`, `subdivide_at_t()`, `curve_intersections()`, `curvature_at_t()`, `uniform_t_by_arclength()`, `sample_at_arclength()`.
- **Section 3:** Shapely silhouette polygon operations.
  - `mask_to_polygon()`, `mask_to_polygon_cv2()`, `polygon_overlap_ratio()`, `polygon_iou()`, `simplify_outline()`, `outline_to_points()`, `polygon_width_profile()`.
- **Section 5:** Audit — `audit_hand_rolled_bezier()` scans codebase for migration status.
- CLI: `--audit`, `--demo`.

### P2: Bezier Audit — 9 files identified
- 8 migratable (bezier3/bezier4/bezier_point → curve_utils imports).
- 1 exempt: rendering_comparison.py (cairo ctx.curve_to, not PIL polyline).
- Files: luma_construction_prototype, luma_gesture_prototype, face_curve_validator, luma_face_curves, luma_expression_sheet, sb_cold_open_P17_chartest, character_lineup, grandma_miri_expression_sheet.

### P3: CI Suite — No changes needed
- No CI check false-flags on bezier/Shapely imports (ext_model_check removed C48).
- Stub linter checks file existence, not third-party imports.

### P4: docs/pil-standards.md updated
- bezier and Shapely added to approved dependency list.
- Migration note added.

### Deliverables
- LTG_TOOL_curve_utils.py v1.0.0 (NEW)
- docs/pil-standards.md updated (bezier + Shapely + migration section)
- README updated (Script Index + C51 section)
- Report sent to Alex Chen inbox
- Archived 2 inbox messages (C50 assignment, C51 assignment)
- Ideabox: curve quality CI check idea submitted

### Lessons Learned
- The bezier library's `evaluate_multi()` is the key function — it vectorizes point sampling across t values, which is significantly faster than Python loops for high step counts. The hand-rolled versions iterate per-step in pure Python.
- Shapely's polygon operations (intersection, union) are resolution-independent, unlike pixel-level mask operations. This means silhouette comparison can work at any scale without rasterizing first — important for QA that needs to be fast.
- Graceful degradation (try/except ImportError with fallback) is essential for shared libraries. If any agent's environment doesn't have the library installed, the tool still works for basic operations.

## New Dependencies (C51)
- **bezier** (`pip install bezier`) — proper bezier curve math. Wrapped by LTG_TOOL_curve_utils.py.
- **Shapely** (`pip install Shapely`) — geometric polygon operations. Wrapped by LTG_TOOL_curve_utils.py.
