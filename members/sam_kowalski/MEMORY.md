...
- **LTG_TOOL_warmcool_scene_calibrate.py v1.0.0 built and deployed.** Measures warm/cool separation in reference photos using same metric as render_qa `_check_warm_cool()`. Batch mode, report generation, threshold override. Registered in tools/README.md.
- **REAL_INTERIOR threshold (12.0) CHALLENGED by reference photos.** 6 out of 7 real interior reference photos fall below 12.0 (median separation: 2.30 PIL hue units). Root cause: warm-lit interiors are warm *everywhere* — the top-half vs bottom-half median hue metric reads near-zero when both halves share the same warm temperature. This is the same systematic false positive class as FP-006/FP-007. The metric works for mixed-temperature scenes but fails for single-temperature-dominant interiors.
- **One outlier at 123.4 separation** (city-view living room at night) — near-50/50 warm/cool split due to large cool exterior visible through window. This is the only reference photo that validates the threshold, and it is not representative of typical warm-lit interior scenes in the show (SF01, SF04, SF05, SF06).
- **AVIF not supported.** 3 of 10 reference photos are .avif format. PIL cannot read them without `pillow-avif` package. Tool skips gracefully. If more AVIF references arrive, request pillow-avif install or convert to JPEG before analysis.
