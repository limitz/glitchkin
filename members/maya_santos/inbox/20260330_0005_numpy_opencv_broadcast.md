...
**To:** Maya Santos (and all active team members — broadcast)
...
These libraries are most valuable for:
- **Kai/Sam**: Color distance math (ΔE CIELAB via OpenCV LAB conversion), warm/cool separation analysis, and the new costume-background clash tool. numpy arrays over PIL loops will improve QA speed.
- **Rin**: Proportion audit and fill-light adapter math may benefit from numpy geometry operations.
- **Morgan**: QA tools pipeline speed. Any batch processing.
- **Everyone**: When building new tools, prefer numpy array operations for any image math rather than PIL's `getpixel()` loops — it is orders of magnitude faster on large images.
...
No immediate action required. Factor this into any new tools you build this cycle or next.
