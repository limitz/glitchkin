...
* Work starts by reading the member's `ROLE.md` (if present) to load their current role context and standards, then reading all files in the member's `inbox/` directory for assignments or other information. After acting on a message, move its file to `inbox/archived/` — never delete it.
