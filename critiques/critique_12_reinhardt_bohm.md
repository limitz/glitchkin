...
1. `output/tools/LTG_CHAR_luma_expression_sheet.py`
2. `output/tools/LTG_CHAR_luma_expression_sheet.py`
3. `output/tools/LTG_CHAR_luma_expression_sheet.py`
4. `output/tools/LTG_CHAR_luma_expression_sheet_v005.py`
5. `output/tools/LTG_CHAR_luma_expression_sheet_v006.py`
6. `output/tools/LTG_CHAR_byte_expression_sheet.py`
7. `output/tools/LTG_CHAR_cosmo_expression_sheet.py`
...
22. `output/backgrounds/environments/LTG_ENV_grandma_kitchen.py`
23. `output/backgrounds/environments/LTG_ENV_tech_den.py`
24. `output/backgrounds/environments/bg_glitch_layer_encounter.py`
25. `output/characters/main/LTG_CHAR_byte_expression_sheet.py`
26. `output/characters/main/LTG_CHAR_cosmo_expression_sheet.py`
27. `output/characters/main/LTG_CHAR_luma_expression_sheet.py`
28. `output/color/style_frames/LTG_TOOL_style_frame_02_glitch_storm.py`
...
48. `output/tools/test_face_lighting.png`
49. `output/tools/test_procedural_draw.png`
...
**Three generators selected:** `LTG_TOOL_luma_expression_sheet.py`, `LTG_TOOL_bg_tech_den.py`, `LTG_TOOL_colorkey_glitchstorm_gen.py`

**Rating: WARN**

### 3.1 — Color Values as Named Constants

**LTG_TOOL_luma_expression_sheet.py:** All palette colors are named constants at the top of the file. No inline tuples found in the palette section. PASS.

**LTG_TOOL_bg_tech_den.py:** All palette colors are named constants. The file defines approximately 60 named color constants before any drawing code. No inline color tuples visible in the reviewed portion. PASS.

**LTG_TOOL_colorkey_glitchstorm_gen.py:** All palette colors are named constants. The comment block explaining the ENV-06 correction is thorough and traceable. PASS.

All three generators comply with the named-constant requirement.

### 3.2 — File Paths: Hardcoded vs. Relative

**LTG_TOOL_luma_expression_sheet.py:** Output path is constructed using `os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "characters", "main")` — relative to the script location. This is correct. The script will produce the right output regardless of working directory. PASS.

**LTG_TOOL_bg_tech_den.py:** Output path is hardcoded as `"/home/wipkat/team/output/backgrounds/environments/LTG_ENV_tech_den.png"`. This is a **hardcoded absolute path**. The script will break or produce output in the wrong location on any system that is not `/home/wipkat/team/`. This is a portability and reproducibility violation. **WARN — hardcoded absolute path.**

**LTG_TOOL_colorkey_glitchstorm_gen.py:** `OUTPUT_DIR` is defined as the hardcoded absolute path `"/home/wipkat/team/output/color/color_keys/thumbnails"`. Same problem as tech den. **WARN — hardcoded absolute path.**
...
**LTG_TOOL_luma_expression_sheet.py:** Output filename is `LTG_CHAR_luma_expression_sheet.png`. This is the correct output asset name. PASS.

**LTG_TOOL_bg_tech_den.py:** Output filename is `LTG_ENV_tech_den.png`. This matches the expected asset. PASS.

**LTG_TOOL_colorkey_glitchstorm_gen.py:** Output filename is `LTG_COLOR_colorkey_glitchstorm.png`. This matches the expected asset. PASS.

### 3.4 — Seeded RNG / Reproducibility

**LTG_TOOL_luma_expression_sheet.py:** No random elements visible in the reviewed code — character geometry is deterministic. PASS.

**LTG_TOOL_bg_tech_den.py:** Uses `rng = random.Random(42)` — module-level seeded RNG. PASS.

**LTG_TOOL_colorkey_glitchstorm_gen.py:** Uses `random.seed(202612)` called inside the draw function. PASS.
...
- `output/color/style_frames/LTG_COLOR_styleframe_otherside.png` — a v004 version of SF03 exists on disk, not listed in the index at all. The index lists v003 as canonical. Either v004 is an error or it supersedes v003 and the index has not been updated.
- `output/color/style_frames/LTG_COLOR_styleframe_luma_byte.png` — SF04 v002 exists on disk. The index lists only v001 as delivered (Cycle 25). v002 is not mentioned anywhere in the index.
- `output/characters/main/LTG_CHAR_luma_expression_sheet.png` — listed in the index as the current sheet (Cycle 26). The index entry exists. However, `LTG_CHAR_luma_expression_sheet_v006.py` exists in `output/tools/`, implying a v006 output may have been generated. If so, the index is stale by one version.
...
17. `LTG_TOOL_character_lineup.py`
18. `LTG_TOOL_character_lineup.py`
19. `LTG_TOOL_luma_classroom_pose.py`
20. `LTG_TOOL_sb_act2_contact_sheet.py`
21. `LTG_TOOL_sb_act1_full_contact_sheet.py`
22. `LTG_TOOL_miri_turnaround.py`
23. `LTG_TOOL_fidelity_check_c24.py`
24. `LTG_TOOL_procedural_draw.py`

That is 24 tools absent from the registry.

### 5.2 — README References Tools Renamed but Still Lists Old Names as Active

The README entry for `LTG_TOOL_luma_expression_sheet.py` states it "Previously misnamed `LTG_CHAR_luma_expression_sheet.py` — renamed in Cycle 22." However, `LTG_CHAR_luma_expression_sheet.py` still exists on disk in `output/tools/`. The rename was not a rename — both files co-exist. Same situation for `LTG_CHAR_luma_expression_sheet.py`, `LTG_CHAR_luma_expression_sheet.py`, `LTG_CHAR_byte_expression_sheet.py`, and `LTG_CHAR_cosmo_expression_sheet.py`. The README describes a state of the repository that does not exist.
...
|---|---|---|---|
| 1 | `style_guide.md` vs `character_sheet_standards.md` | Conflicting head count specification (3.5–4 HU in style guide; 4.5–5.0 HU in standards) | CRITICAL |
...
| 15 | `LTG_TOOL_bg_tech_den.py` | Hardcoded absolute output path | WARN |
| 16 | `LTG_TOOL_colorkey_glitchstorm_gen.py` | Hardcoded absolute output path | WARN |
| 17 | `pitch_package_index.md` | Header states Cycle 24; document contains Cycle 26 additions | WARN |
| 18 | `pitch_package_index.md` | `LTG_COLOR_styleframe_otherside.png` on disk, not indexed | WARN |
| 19 | `pitch_package_index.md` | `LTG_COLOR_styleframe_luma_byte.png` on disk, not indexed | WARN |
...
**On generators:** The hardcoded absolute path pattern is the most serious technical finding. `LTG_TOOL_luma_expression_sheet.py` demonstrates the correct pattern. Use `os.path.dirname(os.path.abspath(__file__))` plus relative path construction. Apply this to every generator that currently hardcodes `/home/wipkat/team/`. This is a one-line fix per file.

**On the pitch package index:** The untracked v004 of SF03 (`LTG_COLOR_styleframe_otherside.png`) and v002 of SF04 (`LTG_COLOR_styleframe_luma_byte.png`) are the most urgent gaps. If a studio contact receives the pitch package and asks "is this the current version," the answer depends on files that are not in the index. The index's purpose is to prevent exactly that ambiguity.
