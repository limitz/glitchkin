...
**Fix:** `LTG_TOOL_luma_color_model.py` generated. Eye width corrected to `int(head_r * 0.22)` = ~10px. Cheek nubs added to head (classroom-style). Head label updated to "3.2 heads". Output: `LTG_COLOR_luma_color_model.png`.
...
`LTG_TOOL_cosmo_expression_sheet.py` is byte-identical to v003 and outputs `LTG_CHAR_cosmo_expression_sheet.png`. The v004 PNG file exists independently.

**Status:** KNOWN DEFECT — not fixed in C30 (requires rebuild to resolve properly). Documented in character_sheet_standards.md inconsistency log.

---

## 5. Miri v003 Stub — Broken Generator

`LTG_TOOL_grandma_miri_expression_sheet.py` is a stub that imports from `LTG_CHAR_grandma_miri_expression_sheet_v003` which no longer exists. The PNG `LTG_CHAR_grandma_miri_expression_sheet.png` was generated before the rename and exists correctly.
...
`output/production/character_sheet_standards.md` Section 7 updated to reflect actual current sheet versions:
- Luma: v007 (was v003)
- Cosmo: v004 (was v003)
- Byte: v004 (was v003)
- Miri: v003 (was v002)
