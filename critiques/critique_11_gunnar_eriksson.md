...
| Line weight standard | COMPLIANT | 3-tier system (silhouette 4px / interior ~2px / detail ~1–1.5px at output) documented in `character_sheet_standards.md`. Brow weight error (Cycle 19 fix: 10px→4px) resolved. Current sheets compliant. |
...
| Construction guide policy | COMPLIANT | Policy is clear: OFF by default in pitch output, ON only for production reference. `LTG_CHAR_luma_expression_sheet_guides.png` correctly labelled as internal-only. |
...
| Legacy filenames coexisting with LTG copies | NON-COMPLIANT | `bg_glitch_layer_encounter.png` (no LTG prefix) coexists with `LTG_ENV_glitchlayer_encounter.png`. `frame01_house_interior.png` coexists with `LTG_ENV_lumashome_study_interior.png`. `glitch_layer_frame.png` coexists with `LTG_ENV_glitchlayer_frame.png`. These are duplicates: non-LTG originals and LTG-compliant copies exist side by side. Originals have not been archived. Three ENV environments in this state. |
| Superseded environment versions present | NON-COMPLIANT | `LTG_ENV_tech_den.png`, `v002.png`, `v003.png` remain on disk alongside canonical `v004.png`. Same pattern for `grandma_kitchen` v001/v002/v003, `school_hallway` v001/v002, `glitch_layer_frame` v001/v002/v003. No archival separation. At production scale with multiple artists per environment, this version proliferation is a continuous source of artist error — the wrong version will be used. |
...
| `LTG_TOOL_style_frame_02_glitch_storm.py` in style_frames directory | NON-COMPLIANT | A generator script stored in the style frames output folder. Tools belong in `output/tools/`. |
...
| Non-LTG storyboard documents | NON-COMPLIANT | `ep01_cold_open.md`, `act2_thumbnail_plan.md`, `act2_thumbnail_plan.md` do not use LTG naming. These are production reference documents (`LTG_SB_*` or `LTG_PROD_*` format expected). |
...
| `character_sheet_standards.md` | NON-COMPLIANT | Expected category: `PROD`. Expected name: `LTG_PROD_character_sheet_standards.md`. Note: the document's own footer cites itself as `LTG_PROD_character_sheet_standards.md` — the intended correct name. The actual filename on disk does not match its own stated name. |
...
| `ltg_pitch_brief.md` | NON-COMPLIANT | Non-standard prefix `ltg_` (lowercase) instead of `LTG_`. Expected: `LTG_PROD_pitch_brief_v001.md`. |
...
| `LTG_BRAND_logo_asymmetric.png`, `v002.png` | COMPLIANT | Two LTG-branded logo files. LTG-named, versioned. |
...
| Remaining `LTG_CHAR_*` generators in tools/ | NON-COMPLIANT | `LTG_CHAR_luma_expression_sheet.py`, `LTG_CHAR_luma_expression_sheet.py`, `LTG_CHAR_luma_expression_sheet.py`, `LTG_CHAR_byte_expression_sheet.py`, `LTG_CHAR_cosmo_expression_sheet.py`, `LTG_CHAR_glitch_expression_sheet_v001.py`, `LTG_CHAR_glitch_expression_sheet_v002.py`, `LTG_CHAR_glitch_turnaround_v001.py`, `LTG_CHAR_glitch_color_model_v001.py` are in the tools directory but use `CHAR` category prefix rather than `TOOL`. Cycle 22 renamed Luma v002/v003/v004 and Byte v004 and Cosmo v004 — the README states this. However, the old filenames remain on disk in `output/tools/`. The renamed versions are also present. This means both `LTG_CHAR_luma_expression_sheet.py` AND `LTG_TOOL_luma_expression_sheet.py` exist. Double files. Rename executed but originals not deleted. |
...
|---|---|---|---|
| NC-01 | Glitch expression sheet canvas non-compliance | `LTG_CHAR_glitch_expression_sheet.png` | 800×800 / 2×2 grid violates `character_sheet_standards.md` Section 5 mandate of 1200×900. Current sheet is under-spec for its required production function. A buyer cannot assess character expressiveness at this canvas. |
| NC-02 | No visual color models for three lead characters | (missing) `LTG_COLOR_luma_color_model.png`, `LTG_COLOR_byte_color_model.png`, `LTG_COLOR_cosmo_color_model.png` | Color model PNGs for Grandma Miri and Glitch exist. Luma, Byte, Cosmo have `.md` specs only. A new artist or studio colorist cannot verify production compliance from a markdown file. The three-lead asymmetry is a structural gap. |
...
| NC-04 | `LTG_CHAR_lineup.png` undocumented | `LTG_CHAR_lineup.png` | File exists on disk. Pitch package index lists Glitch addition to lineup as a Cycle 24 LOWER priority open item. If the file was generated before official index documentation, it is pre-canonical content in the main production directory. If it contains Glitch, the character lineup section of the index needs immediate update. Either way: undocumented asset in pitch package directory. |

### SEVERITY 2 — PIPELINE HYGIENE FAILURES

| ID | Item | File(s) | Issue |
|---|---|---|---|
| NC-05 | Generator scripts in output asset directories | `bg_glitch_layer_encounter.py` (in `environments/`), `LTG_TOOL_style_frame_02_glitch_storm.py` (in `style_frames/`) | Tools must be in `output/tools/`. Source code in asset output directories contaminates the output structure. Artists and compositors searching for assets encounter scripts. |
...
| NC-09 | Renamed generator files not cleaned up | `LTG_CHAR_luma_expression_sheet.py`, `v003.py`, `v004.py`, `LTG_CHAR_byte_expression_sheet.py`, `LTG_CHAR_cosmo_expression_sheet.py` (in `output/tools/`) | Cycle 22 rename created `LTG_TOOL_*` versions. The `LTG_CHAR_*` originals remain. Both exist. The rename was executed; the cleanup was not. Additionally: `LTG_CHAR_glitch_expression_sheet_v001.py`, `v002.py`, `LTG_CHAR_glitch_turnaround_v001.py`, `LTG_CHAR_glitch_color_model_v001.py` were never renamed — these are Glitch character generators that post-date the CHAR→TOOL rename directive. |
| NC-10 | Superseded character sheets not archived | `LTG_CHAR_luma_expression_sheet.png`, `v003.png`, `LTG_CHAR_byte_expression_sheet.png`, `v002.png`, `v003.png`, `LTG_CHAR_cosmo_expression_sheet.png`, `v002.png`, `v003.png`, `LTG_CHAR_grandma_miri_expression_sheet.png` (in `characters/main/`) | Superseded versions coexist with canonical current versions in the same directory. At production scale, this creates version selection errors. The index says "use v003" or "use v004" — but a new artist reading a file listing has no visual cue of canonical status. |
...
| ID | Item | File(s) | Issue |
|---|---|---|---|
| NC-13 | All production documents use non-LTG naming | `production_bible.md`, `naming_conventions.md`, `naming_convention_compliance_checklist.md`, `character_sheet_standards.md`, `style_guide.md`, `master_palette.md`, `byte_float_physics.md`, `fx_spec_cold_open.md`, `fx_confetti_density_scale.md`, `corruption_visual_brief.md`, `storyboard_pitch_export.png`, `show_logo.png`, `ltg_pitch_brief.md`, all statement_of_work files, all critique files | Zero production documents use the `LTG_[CATEGORY]_[descriptor]_v[###].[ext]` format. The naming system has been mandatory from Cycle 13. Not one production document was retroactively renamed. The naming conventions document cannot be enforced by a system that the document itself violates. |
...
| Asset | File | State |
|---|---|---|
| Luma expression sheet | `LTG_CHAR_luma_expression_sheet.png` | PRODUCTION READY |
| Byte expression sheet | `LTG_CHAR_byte_expression_sheet.png` | PRODUCTION READY |
| Cosmo expression sheet | `LTG_CHAR_cosmo_expression_sheet.png` | PRODUCTION READY |
| Grandma Miri expression sheet | `LTG_CHAR_grandma_miri_expression_sheet.png` | PRODUCTION READY |
| Character lineup | `LTG_CHAR_lineup.png` | PRODUCTION READY |
| Grandma Miri color model (visual) | `LTG_COLOR_grandma_miri_color_model.png` | PRODUCTION READY |
| SF01 Discovery | `LTG_COLOR_styleframe_discovery.png` (and styled variant) | PRODUCTION READY |
| SF02 Glitch Storm | `LTG_COLOR_styleframe_glitch_storm.png` | PRODUCTION READY (non-stylized confirmed) |
| SF03 Other Side | `LTG_COLOR_styleframe_otherside.png` | PRODUCTION READY (non-stylized confirmed) |
| Grandma Kitchen | `LTG_ENV_grandma_kitchen.png` | PRODUCTION READY |
| Tech Den | `LTG_ENV_tech_den.png` | PRODUCTION READY |
| School Hallway | `LTG_ENV_school_hallway.png` | PRODUCTION READY |
| Millbrook Main Street | `LTG_ENV_millbrook_main_street.png` | PRODUCTION READY |
| Classroom BG | `LTG_ENV_classroom_bg.png` | PRODUCTION READY |
| Glitch Layer frame | `LTG_ENV_glitchlayer_frame.png` | PRODUCTION READY |
| Other Side BG | `LTG_ENV_other_side_bg.png` | PRODUCTION READY |
| Act 2 storyboard contact sheet | `LTG_SB_act2_contact_sheet.png` (or v006) | PRODUCTION READY |
| LTG brand logos | `LTG_BRAND_logo_asymmetric.png`, `LTG_BRAND_logo_asymmetric.png` | PRODUCTION READY |
| Master palette content | `output/color/palettes/master_palette.md` | CONTENT: PRODUCTION READY. Filename: non-compliant. |
| Character sheet standards | `output/production/character_sheet_standards.md` | CONTENT: PRODUCTION READY. Filename: non-compliant. |
| Render library | `LTG_TOOL_render_lib.py` | PRODUCTION READY |
| Stylization tool | `LTG_TOOL_stylize_handdrawn.py` | PRODUCTION READY |
...
4. Verify, document, or archive `LTG_CHAR_lineup.png` (NC-04).
