...
- LTG_ENV_grandma_living_room.png (C37), LTG_SB_pilot_cold_open.png (C37), LTG_CHAR_luma_motion.png (C37), LTG_CHAR_byte_motion.png (C37), story_bible_v001.md (C37) — zero of these appear in the index.
...
- Root cause: `spec_sync_ci_v001` calls `glitch_spec_lint_v001` without loading `glitch_spec_suppressions.json`, so the G002 self-test false-positive in `LTG_TOOL_glitch_spec_lint.py` escalates to a P1 violation. The suppression list that resolves this was not wired into the CI gate.
...
- `LTG_TOOL_readme_sync.py` (live run) reports 1 UNLISTED tool: `LTG_TOOL_color_qa_c37_runner.py`. The C37 precritique_qa report claimed 0 UNLISTED. The README was not in sync when critique began.
...
- VIOLATION: `LTG_CHAR_luma_motion.py` and `LTG_CHAR_byte_motion.py` are stored in `output/tools/` but use the `CHAR` category prefix. Per naming_conventions.md, scripts in the tools directory must use `LTG_TOOL_` prefix. The approved CHAR category is for design sheets and model assets, not generator scripts.
- VIOLATION: `LTG_SB_pilot_cold_open.py` uses the `SB` (storyboard) category for a generator script — also a naming convention violation for a tool file.
...
- Lineup file `LTG_CHAR_character_lineup.png` is confirmed on disk as the current version; `LTG_CHAR_lineup_v007.png` is the README-listed output filename — these differ by descriptor (`character_lineup` vs `lineup`). Inconsistent.
...
- `LTG_TOOL_spec_extractor.py luma` returns HEAD RATIO: NOT FOUND, EYE COEFF: NOT FOUND. The extractor is reading `luma_expression_sheet_metadata.md` (not `luma.md`). This is a tool configuration issue — the extractor cannot confirm Luma's canonical proportions from the spec file it targets.
...
- `LTG_SB_pilot_cold_open.py`: draw_order_lint_v002 returns **W004** (stale draw object after img.paste at line 115). A stale draw object means all subsequent draw calls may not register on the composited layer — this is a functional defect in the generator, not an advisory. The output PNG may be incorrect in ways that are invisible at thumbnail scale.
