...
Reproducibility test applied to Luma character design (primary asset). New-artist assumption: read `luma.md`, `master_palette.md`, `naming_conventions.md`, and `character_sheet_standards.md` — attempt to produce a compliant character sheet without asking questions. Counted every decision left undefined or contradicted.
...
- `run_c31_qa.py` and `test_face_lighting.py` in `output/tools/` active directory are non-compliant. Both are utility/test scripts and must carry `LTG_TOOL_` prefix; they are indexed in `README.md` as PASS by the linter because they aren't in the CHAR/COLOR/BRAND-violation class, but they violate the naming convention stated in `naming_conventions.md` Section "Tool / Script Assets."
- `LTG_TOOL_naming_compliance_copy.py` vs `LTG_TOOL_naming_compliance_copier/v002.py` — three tools with overlapping purpose, no consolidation decision recorded. README says "flagged for consolidation review — Alex Chen to decide" — C31 and still unresolved.
...
## System 2 — Draw-Order Lint (LTG_TOOL_draw_order_lint.py)
...
## System 3 — character_sheet_standards.md
...
- Cosmo v004 issue is production-critical: `LTG_TOOL_cosmo_expression_sheet.py` is byte-identical to v003 and outputs `_v003.png`. A new artist running v004 gets a v003 output and a naming collision — this is not a flag for discussion, it is a defect requiring a fix.
...
**Gap 1 (CRITICAL — direct contradiction):** `luma.md` Section 3 states Luma's total height is **3.5 heads**. `character_sheet_standards.md` Section 7 states the current sheet (v007, C29) uses **3.2 heads**. Both documents are active. A new artist must guess which is correct. There is no document that records the decision to change this value or supersedes `luma.md`.

**Gap 2 (CRITICAL — broken toolchain):** `LTG_TOOL_luma_expression_sheet.py` is a forwarding stub that imports from `LTG_CHAR_luma_expression_sheet_v006`. That module does not exist on disk. Running the stub throws `ModuleNotFoundError`. The stub is non-functional. The same issue likely applies to other C28 forwarding stubs (`cosmo_turnaround_v002`, `luma_turnaround_v002`, `grandma_miri_expression_sheet_v003`, `luma_expression_sheet_v005`). I tested one; it fails immediately.

**Gap 3:** `luma.md` Section 5 specifies **5 curls** (locked production count). `LTG_TOOL_luma_expression_sheet.py` (current canonical) uses "8-ellipse hair curl cloud" per its README entry. 5 vs 8. No document records which standard supersedes the other.
...
**Gap 6:** No document specifies what happens if a Luma expression sheet must be produced for a character size outside the specified HEAD_R=105 (1×). `character_sheet_standards.md` Section 2 says "never set HEAD_R so large that body/legs fall below panel bottom" — but does not specify the minimum HEAD_R before switching to bust format.
...
**Bottom line:** A new artist cannot produce a correct Luma character sheet from documentation alone — the proportion conflict between `luma.md` and `character_sheet_standards.md` is unresolvable without external information, and the current canonical generator (v007) cannot be run because v006's forwarding stub is broken.
...
- **Missing:** No FX or SB asset examples reference actual current assets. The SB section lists hypothetical PDF paths that do not match actual panel file naming (`LTG_SB_ep01_cold_open_seq01_v001.pdf` does not exist; actual panels use `LTG_TOOL_sb_panel_a101.py`). A new artist will be confused about whether the storyboard asset naming examples reflect the real pipeline.
...
- `character_sheet_standards.md` updated with current versions: done.
- `LTG_TOOL_draw_order_lint.py` built: done.
